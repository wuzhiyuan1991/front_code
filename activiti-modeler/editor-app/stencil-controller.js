...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        // Property window toggle state 属性窗口切换状态
        $scope.propertyWindowState = {'collapsed': false};

        // Add reference to global header-config 添加全局头配置引用
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            {//debugger;
            	//设置弹窗的默认参数
                $scope.modal = {
                			//控制背景，允许的值：true（默认），false（无背景），“static” - 背景是存在的，但点击模态窗口之外时，模态窗口不关闭
                			//这里必须是static，因为有弹窗和父窗口的数据交互
                			backdrop:'static'
                		};
                
                //anson tag 初始化相关配置参数
                $http({method: 'GET', url: KISBPM.URL.getBusinessCfg()}).success(function (data, status, headers, config) {
                	//debugger;
                	 //初始化全局的条件、流程角色映射配置信息
    	            $scope.condExpDisplayValueMap = data.condExpDisplayValueMap;
    	            $scope.candidateDisplayValueMap = data.candidateDisplayValueMap;
    	            $scope.processDisplayValueMap = data.processDisplayValueMap;
                    ...
                    for(var key in $scope.processDisplayValueMap) {
                        processGroupAll.push({name:$scope.processDisplayValueMap[key], value:key });
                    };
    	            $scope.processGroupAll = processGroupAll;
                });
                
//	            //初始化全局的条件、流程角色映射配置信息
//	            $scope.condExpDisplayValueMap = {	
//	        			'${status==11}': '无法整改',
//						'${status==200}':'无需整改',
//						'${status==2}': '审批问题',
//						'${status==21}' : '整改受阻'
//						};
//	            $scope.candidateDisplayValueMap = {
//	            		'zhan_zhang_shen_pi':'站长审批角色',
//						'qu_yu_jing_li_shen_pi':'区域经理角色'
//						};
//	            var candidateGroupsAll = [];
//	            var candidateDisplayValueMap = $scope.candidateDisplayValueMap;
//	            for(var key in candidateDisplayValueMap) {
...
//	            $scope.candidateGroupsAll = candidateGroupsAll;
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
           	...
           	// debugger;
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    var stencil = selectedShape.getStencil();//debugger
                    ...
                    var properties = stencil.properties();//debugger
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //anson tag
                            //如果是流程条件，需要修改流程条件的显示，增加valueDisplay属性，
                            //对应的html为：condition-expression-display-template.html
                            var targetKey = "oryx-conditionsequenceflow";
                            console.log(key);//debugger
                            if(targetKey == key) {//debugger
                            	var condExpDisplayValueMap = $scope.condExpDisplayValueMap;
                            	
                            	currentProperty.valueDisplay = condExpDisplayValueMap[currentProperty.value] || currentProperty.value ;
                            };
                            var targetId = "oryx-overrideid";
                            if((key === targetId) && (i === 0) && (properties.length == 18)){
                                if (propertyConfig.readModeTemplateUrl !== undefined && propertyConfig.readModeTemplateUrl !== null) {
                                    currentProperty.readModeTemplateUrl = 'editor-app/configuration/properties/id-display-template.html' + '?version=' + $rootScope.staticIncludeVersion;
                                }
                                if (propertyConfig.writeModeTemplateUrl !== null && propertyConfig.writeModeTemplateUrl !== null) {
                                    currentProperty.writeModeTemplateUrl = 'editor-app/configuration/properties/id-property-template.html' + '?version=' + $rootScope.staticIncludeVersion;
                                }
                            }
                            
                            selectedItem.properties.push(currentProperty);//debugger
                        }
                    }
        ...
        $scope.propertyClicked = function (index) {//debugger
        ...
        $scope.getPropertyWriteModeTemplateUrl = function (index) {//debugger
        ...
        $scope.updatePropertyInModel = function (property, shapeId) {//debugger
