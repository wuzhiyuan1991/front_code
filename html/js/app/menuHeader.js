  ...
  var opts = {
          ...
          columns: [
            ...
            },
            {
              ...
              render: function(data) {
                if (data.status === "2") {
                  return (
                    '<div title=""   style="color: #33adea; cursor: pointer;">' +
                    "下载" +
                    "</div>"
                  );
                } else {
...

            {
              ...
              render: function(data) {
                if (data.sign) {
                  var className =
                    data.sign == "2"
                      ? "status-rect-tag-yellow"
                      : "status-rect-tag-green";
                  var text = data.sign == "2" ? "删除" : "";
                  return (
                    '<div class="status-rect-tag ' +
                    className +
                    '" style="color: #fff;">' +
                    text +
                    "</div>"
    ...
    methods: {
      ...
      doSave: function() {
        ...
        Vue.http
          .post("user/" + LIB.user.id + "/profile/1", params)
          .then(function() {
      ...
      // 切换语言
      changeLangType: function(item) {
        if(window.localStorage.lang !==item){
