define("app/app", function(require) {
  ...
  var startRouter = function(menuList) {
    ...
    var headerMenuData = routerCfg.restructureData(
      menuList,
      "id",
      "parentId",
      "children"
    ...
    var searchModelFunc = function() {
      ...
      var allPath = _.chain(menuList)
        .filter(function(item) {
          return !!item.attr2;
        })
        .map("attr1")
        .value();
      obj.searchList = _.filter(obj.searchList, function(item) {
        return _.contains(allPath, item.path);
      });
    ...
    var App = LIB.Vue.extend({
      ...
      computed: {
        ...
        showWorkState: function() {
          return (
            LIB.user.username != "sysadmin" &&
            LIB.user.username != "superadmin" &&
            LIB.getBusinessSetStateByNamePath("common.enableUserWorkState")
          );
        }
      },
      methods: {
        ...
        doSearch: function() {
          ...
          if (
            ...
            searchData.value = {
              displayName: "关键字",
              filterName: "criteria.strValue.keyWordValue",
              filterValue: this.searchModel.params
            };
        ...
        doLogout: function() {
          LIB.Modal.confirm({
            title: "确定退出登录?",
            onOk: function() {
              var logoutActionUrl = window.localStorage.getItem(
                "logoutActionUrl"
              ); //自定义登出地址
              if (!logoutActionUrl) {
                api.logout().then(
        ...
        group: function() {
          ...
          if (LIB.user.attr4 == 1) {
            window.open(
              LIB.ctxPath(
                "/html/js/app/views/webim/main.html?name=" +
                  name +
                  "&password=" +
                  password
              )
        ...
        doMenuFinshed: function() {
          ...
          if (lazyLoadComponent["menuConfig"] > 0) {
            this.formModal.curFormModal.show = !this.formModal.curFormModal
              .show;
            // LIB.asideMgr.hideAll();
            // this.$broadcast('ev_detailDataReload');
          }
