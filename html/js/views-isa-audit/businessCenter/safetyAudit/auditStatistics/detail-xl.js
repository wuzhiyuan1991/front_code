    ...
    var detail = LIB.Vue.extend({
        ...
        methods: {
            ...
            afterInitData: function() {
                ...
                this.getRates();
                this.viewType = 'table';
                this.$api.getElements({auditTableId: this.mainModel.vo.auditTable.id, elementType: 10, auditCriterion: 5}).then(function(res){
                   if(res.data){
                       _.each(res.data, function(item){
                           _this.bonusScore += parseFloat(item.score);
                       })
                   }
                });
            },
            ...
            doAddLevel: function(){
                ...
                if(len > 0) {
                    ...
                    if(max >= (parseFloat(this.mainModel.vo.auditTable.score) + this.bonusScore)) {
                        LIB.Msg.info("上限分数不能高于审核表总分与加分之和");
                        return;
                    }
                }
