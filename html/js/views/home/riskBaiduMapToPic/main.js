define(function (require) {
    ...
    var riskMapArea = require("../../businessCenter/hiddenDanger/riskMapArea/main");
    ...
    var component = LIB.Vue.extend({
        ...
        methods: {
            ...
            _init: function () {
                ...
                api.getSetting(params).then(function (res) {
                    ...
                    var map = new BMap.Map("allmap", {
                        minZoom:data.minZoom?data.minZoom:dataObj.minZoom,
                        maxZoom: data.maxZoom?data.maxZoom:dataObj.maxZoom,
                    ...
                    if (p.x && p.y) {
                        ...
                        map.centerAndZoom(point, data.defaultZoom?data.defaultZoom:(data.minZoom?data.minZoom:dataObj.minZoom));  // 初始化地图,设置中心点坐标和地图级别
                    } else {
                        map.centerAndZoom(centerCity, data.defaultZoom?data.defaultZoom:(data.minZoom?data.minZoom:dataObj.minZoom));  // 初始化地图,设置中心点坐标和地图级别
                    }
            ...
            _overlayClicked: function (e) {
                ...
                if(this.compMap){
                    ...
                    this.optionObj.name = item.name;
                    this.optionObj.imgSrc = item.imgSrc;
                    this.$route.query.pictureId = item.nextPictureId;
                    this.$route.query.orgId = item.orgId;
                    this.pictureId = item.nextPictureId;
                    this.$router.go(this.$route);
                    this.showPicMap = false;
                    return ;
                }

                if (item.isClick == 1) { //等于1  才能点击进入下一级
                    this.drawOrgId = item.drawOrgId;
            ...
            _getLookUpDataJson: function () {
                var _this = this;
                api.getCompy().then(function (res) {
                    console.log(res.data);
                    _this.initDataCompy([],res.data)
                })
            },
            ...
            initDataCompy: function (companyArr, departmentArr) {
                var _this = this;
                ...
                var arr2 = _.map(departmentArr, function (item) {
                    ...
                    if(!item) return {}
                    return {
                        conf:(item && item.conf) || null ,
                        drawOrgId:item.drawOrgId,
                        name: item.orgName,
                        id: item.drawOrgId,
                        parentId: item.orgId,
                        level: '3',
                        riskLevel: item.riskFormulaBean.level,
                        x: item.points[0].x,
                        y: item.points[0].y,
                        px:item.points[1]?item.points[1].x:null,
                        py:item.points[1]?item.points[1].y:null,
                        realX: item.points[0].x,
                        realY: item.points[0].x,
                        checked: true,
                        defalutLevel:item.riskFormulaBean.level,
                        pictureId:item.pictureId,
                        isClick:'1',
                        orgId:item.orgId,
                        nextPictureId:item.nextPictureId,
                        imgSrc: item.imgSrc
                    }
                });

                this.baseList = [].concat(arr2);
                this._normalizeTreeData();
            },
        ...
        ready: function () {
            ...
            // 一级地址屏蔽 百度地图
            if(this.$route.path.indexOf('notDefaultTemp')>-1){
                console.log('一级地址屏蔽 百度地图')
                this.compMap = true;
                this.showPicMap = false;
                return ;
            }
        ...
        created: function () {
            ...
            if(this.$route.path.indexOf('riskBaiduMapXY')>-1){
                this.compMap = true;
                this._getLookUpDataJson();
            }
            else  this._getLookUpData();
        },
