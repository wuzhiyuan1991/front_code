define(function (require) {
     ...
     * @constructor
     */
    var RM = function (canvas, opts, legends, title, colorMap, conf) {
        this.opts = opts.data;
    ...
    RM.prototype.drawConstant = function () {
        ...
        var color, font;
        if(RM.prototype.fontStyle){
            color = RM.prototype.fontStyle.labelColor;
            font = RM.prototype.fontStyle.labelWeight + (' '+RM.prototype.fontStyle.labelSize) + 'px serif'
        }
        this.drawText(this.title, 25, this.height - 25,color, font);
        // this.drawLegend();
    };
    ...
    function Polygon(option, colorMap) {
        ...
        if(RM.prototype.fontStyle && RM.prototype.fontStyle.opacity){
            this.opacityValue =  RM.prototype.fontStyle.opacity;
        }
    ...
    Polygon.prototype.draw = function (ctx) {
        ...
        ctx.closePath();
        if(this.colorMap[this.level]){
            var color = this.colorMap[this.level]
            if(color == '255,193,42,'){
                ctx.fillStyle = 'rgba(' + '163,109,62,' + this.opacityValue + ')';
            }else{
                ctx.fillStyle = 'rgba(' + this.colorMap[this.level] + this.opacityValue + ')';
            }
        }else{
            ctx.fillStyle = 'rgba(255,255,255,' + this.opacityValue + ')';
        }
        ...
        if(!RM.prototype.fontStyle && !firstPoint.fontColor){
            ctx.font = "900 24px Arial";
            ctx.lineWidth = 3;   // 边框线条粗细 3
            ctx.fillStyle = "#fff";
        }else{
            ctx.font =RM.prototype.fontStyle.fontWeight +' ' + RM.prototype.fontStyle.fontSize+"px Arial";
            if(firstPoint.fontColor){
                ctx.font =firstPoint.fontWeight +' ' + firstPoint.fontSize+"px Arial";
            }
            ctx.lineWidth = RM.prototype.fontStyle.lineWidth;
            ctx.fillStyle =firstPoint.fontColor?firstPoint.fontColor : RM.prototype.fontStyle.fontColor;
        }
        ...
        if((RM.prototype.fontStyle && RM.prototype.fontStyle.center) || firstPoint.center){
            var maxx = _.max( this.points, function (item) {
                return item.x;
            }).x;
            var maxy = _.max( this.points, function (item) {
                return item.y;
            }).y;
            var minx = _.min( this.points, function (item) {
                return item.x;
            }).x;
            var miny = _.min( this.points, function (item) {
                return item.y;
            }).y;
            var width = maxx - minx;
            ...
            for (var i = 0; i<lines.length; i++) {
                ...
                var wsize = parseInt(width / ll);
                if(firstPoint.fontSize ){
                    var offset = parseInt(width - lines[i].length * firstPoint.fontSize)/2;
                    offset = offset>0?offset:0;

                    height = height - (lineArr.length - 1)*firstPoint.fontSize;

                    var weight = firstPoint.fontWeight?firstPoint.fontWeight:RM.prototype.fontStyle.fontWeight;
                    var font = weight + (' ' +  firstPoint.fontSize)  +"px Arial";
                    ctx.font = font;
                }
                else if(wsize-3 >26){
                    var offset = parseInt(width - lines[i].length * 26)/2;
                    offset = offset>0?offset:0;
                    ctx.font =RM.prototype.fontStyle.fontWeight +'  '+ "26px " +" Arial";
                }else if(wsize <16){
                    var offset = parseInt(width - lines[i].length * 16)/2;
                    offset = offset>0?offset:0;
                    ctx.font =RM.prototype.fontStyle.fontWeight +'  '+ "16px " +" Arial";
                }else{
                    var offset = parseInt(width - lines[i].length * RM.prototype.fontStyle.fontSize)/2;
                    offset = offset>0?offset:0;
                }
                for(var i=0; i<lineArr.length; i++){
                    var fontSize =firstPoint.fontSize || 0;
                    ctx.fillText(lineArr[i],  minx + offset, parseInt(height/2) + miny + 5 + i*fontSize);
                }
            }
        }else{
            if(firstPoint.fontSize || (RM.prototype.fontStyle.fontSize)){
                var weight = firstPoint.fontWeight?firstPoint.fontWeight:RM.prototype.fontStyle.fontWeight;
                var size = firstPoint.fontSize?firstPoint.fontSize : RM.prototype.fontStyle.fontSize;
                var font = weight + (' ' +  firstPoint.fontSize)  +"px Arial";
                ctx.font = font;
                // ctx.font =firstPoint.fontWeight?firstPoint.fontWeight:700 +' ' + firstPoint.fontSize +  +"px Arial";
            }
            for (var i = 0; i<lines.length; i++) {
    ...
    Polygon.prototype.stroke = function (ctx, areaId) {
        ...
        ctx.lineWidth = RM.prototype.fontStyle?RM.prototype.fontStyle.lineWidth : 3;
        ctx.beginPath();
        ctx.moveTo(firstPoint.x, firstPoint.y);
