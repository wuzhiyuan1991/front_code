    ...
    var home = LIB.Vue.extend({
        ...
        methods: {
            ...
            doRenderBgColor: function (riskLevel) {
                var color = helper.doRenderBgColor(riskLevel, this.riskColorMap);
                if(color.backgroundColor == "#fff"){
                    color.color = '#666';
                }
                return color;
            },
            ...
            _getDominationAreas: function (orgId, areaId, levelId) {
                var _this = this;
                api.getStaticAreaList({drawOrgId: orgId, areaId: areaId}).then(function (res) {
                    _this.dominationAreas = [_.find(res.data, function (item) {
                        return   item.id == levelId;
                    })];



                    _this._init(_this.dominationAreas);
                });
            },
            ...
            _getCheckTableItems: function () {
                ...
                api.getCheckItems({
                    ...
                    // 按照风险等级从高到低排序
                    var arr = items.sort(function (a, b) {
                    ...
                    _.each(arr, function (arrItem) {
                        var obj = _.find(list, function (listItem) {
                            return listItem.levelOfControl==arrItem.levelOfControl &&
                                listItem.scene==arrItem.scene &&
                                listItem.riskLevel==arrItem.riskLevel &&
                                ((!listItem.hazardFactor && !arrItem.hazardFactor)  || ((listItem.hazardFactor && arrItem.hazardFactor) && ((listItem.hazardFactor.name == arrItem.hazardFactor.name))))
                        });
                        if(!obj){
                            list.push(arrItem);
                        }
            ...
            _updateRiskLevel: function(data) {
                ...
                if (parseInt(this.dynamicRiskLevel) < parseInt(latestLevel)) {
                    this.dynamicRiskLevel = latestLevel;
                }
            ...
            init: function (data) {
                ...
                this._getDominationAreas(data.drawOrgId, "L1", data.areaId);
                // this._init([{id:data.areaId, name:''}]);
                ...
                this.staticRisk = '1';
                this.dynamicRisk = '1';
                this.showRightDynamicRisk = data.riskType === 'dynamic' ? true : false;
            },
