    ...
    var component = LIB.Vue.extend({
        ...
        methods: {
            ...
            init: function (params) {
                ...
                if (_.isArray(params)) {
                    this.mainModel.vo.compId = params[0].compId;
                    this.mainModel.vo.course.id = params[0].course.id;
                    this.mainModel.vo.course.name = params[0].course.name;

                    this._checkCourseExamExist(this.mainModel.vo.course.id);
                    this.users = _.map(params, function (item) {
                        return {
                            id: item.userId,
                            name: item.userName
                        }
                    });
                    this.mainModel.vo.trainTasks = _.map(params, function (item) {
                        return {
                            compId: item.compId,
                            orgId: item.orgId,
                            user: { id: item.userId }
            ...
            doSave: function (status) {
                ...
                this.$refs.ruleform.validate(function(valid) {
                    if (valid) {
                        api.createRetrainPlan(param).then(function (res) {
