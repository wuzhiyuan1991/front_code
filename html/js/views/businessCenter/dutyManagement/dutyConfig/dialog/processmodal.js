    ...
    var opts = {
        ...
        computed: {
            filterData: function () {
                if(this.mainModel.vo.id){
                    return {
                        "criteria.strValue.orgId": this.orgId,
                        source: '1',
                        "criteria.strsValue.excludedIds": [this.mainModel.vo.id]
                    }
                }else{
                    return { "criteria.strValue.orgId": this.orgId , source: '1' }
                }
            ...
            styleObj:function(){
                var obj={}
                debugger
                if(this.editor && this.editor.selectedElement) {
        ...
        ready: function () {
            this.initData({lines:[],nodes:[]})
                ...
                document.addEventListener("keydown", this.doDel);
        ...
        methods: {
            ...
            doSave: function (ctx, data) {
                var _this = this;
                _this.refreshData(JSON.parse(data));
            ...
            doSaveClose: function () {
                ...
                setTimeout(function () {
                    if(_this.mainModel.vo.content.length < 25) return LIB.Msg.info('模板为空,不能保存');
            ...
            doDel:function(e){
                if(!this.editor.selectedElement) return  LIB.Msg.info("请选中模块操作!");
                if (e.keyCode==46) {
                    $('#delEditor').click()
                } 
        ...
        destroyed:function(){
            document.removeEventListener("keydown", this.doDel);
        }
    };
