...
//            	}
            ...
            doEvaluate:function () {
                ...
                if(deleteIds.length > 1){
                    LIB.Msg.warning("无法批量修改评价");
                    return;
                } else {
                    this.evaluateModal.show=true;
                }
            },

            doUpdateEvaluate:function (nVal) {
                var _this = this;
                api.updateEvaluate({type:nVal}, this.tableModel.selectedDatas[0]).then(function (data) {
                    _this.refreshMainTable();
                    LIB.Msg.success("修改成功");
                });
                this.evaluateModal.show=false;
            }
        },
