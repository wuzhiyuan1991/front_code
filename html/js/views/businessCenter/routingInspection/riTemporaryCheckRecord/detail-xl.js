		...
		methods:{
            ...
            doExportExcel: function () {
                ...
                LIB.Modal.confirm({
                    title: '导出数据?',
                    onOk: function () {
                        window.open('/richeckrecord/' + id + '/temporary/exportExcel');
                    }
                });
            },
			afterInitData : function() {
				...
				api.queryRiCheckRecordDetails({id: this.mainModel.vo.id}).then(function (res) {
                    _this.tableOrgId = _.get(res, "data.riCheckTable.orgId");
            ...
            _convertCheckAreaArray: function (items) {
                ...
                _.each(items, function (item) {
                    if(item.orgId == _this.tableOrgId) {
                        arr.push({
                            name: item.name
                        })
                    }
                ...
                _.each(items, function (item) {
                    if(item.orgId == _this.tableOrgId){
                        ...
                        _.each(item.riCheckPointTpls, function (point) {
                            if(point.orgId == _this.tableOrgId) {
                                ret.points.push({
