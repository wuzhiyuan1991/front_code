define(function(require){
    ...
    var detail = LIB.Vue.extend({
        ...
        computed: {
            getTitle: function () {
                if(this.vo.type == '1') return '电气第一种工作票';
                if(this.vo.type == '2') return '电气第二种工作票';
            ...
            getWorks: function () {
                ...
                if(this.vo && this.vo.ewWorkers){
                    arr = _.sortBy(this.vo.ewWorkers, function (item) {
                        return parseInt(item.type);
                    });
                    str = _.pluck(arr,'name').join('，')
                }
                return str;
            },
            orgName: function () {
            ...
            itemAllRow: function () {
                var num = 11;
                if(this.vo.ewWorkItems) num+=this.vo.ewWorkItems.length;
                return num;
            },
            ewWorkRecordsList: function () {
                var arr = [];
                for(var i=0; i < this.vo.ewWorkRecords.length; i+=2){
                    var temp = [];
                    temp.push(this.vo.ewWorkRecords[i]);
                    if(this.vo.ewWorkRecords[i+1]){
                        temp.push(this.vo.ewWorkRecords[i+1]);
                    }
                    arr.push(temp);
                }
                return arr;
        ...
        methods:{
            ...
            deelList: function () {
                var _this = this;
                var res = {data: {list : this.vo.ewWorkItems}};
