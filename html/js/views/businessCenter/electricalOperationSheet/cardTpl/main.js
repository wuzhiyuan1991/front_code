    ...
    var initDataModel = function () {
        return {
            ...
            tableModel: LIB.Opts.extendMainTableOpt(
                {
                    url: "ewcardtpl/list{/curPage}{/pageSize}",
                    selectedDatas: [],
                    columns: [
    ...
    var vm = LIB.VueEx.extend({
        ...
        computed: {
            isShowShare: function () {
                if(this.tableModel.selectedDatas[0] && this.tableModel.selectedDatas[0].isShare == '0'){
                    return true;
                }
                return false;
            }
        ...
        methods: {
            doShare: function () {
                ...
                var _vo = _.cloneDeep( this.tableModel.selectedDatas[0]);
                var data =  _vo;
                if (data.disable === '1') {
                    return LIB.Msg.warning("未启用的操作票不能进行分享操作");
                }
                _vo.isShare == '1'?_vo.isShare='0':_vo.isShare='1';
                _this.$api.update(_vo).then(function(res) {
                    _this.tableModel.selectedDatas[0].isShare = _vo.isShare;
                    LIB.Msg.info((_this.tableModel.selectedDatas[0].isShare === "0") ? "未分享" : "已分享");
