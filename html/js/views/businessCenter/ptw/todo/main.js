define(function (require) {
  ...
  var authorizeModal = require("./dialog/authorizeModal");
  ...
  var vm = LIB.VueEx.extend({
    ...
    watch: {
      "tableModel.selectedDatas": function (val) {
        ...
        this.authorizeModel.vo = val[0];
        this.checkAuthoriser();
      }
    },
    methods: {
      doSaveAuthorize: function (val) {
        var _this = this;
        api.savePtwDelegateRecord({id: this.mainModel.vo.lastPermitId},{
            workCardId: this.mainModel.vo.id ,
            workPermitId: this.mainModel.vo.lastPermitId,
            type: 1,
            mandatorId: LIB.user.id,
            assigneeId: val.principals[0].id,
            relId: val.relId,
            reason: val.auditOpinion
        }).then(function (res) {
            LIB.Msg.info("委托成功");
      ...
      showApprovalModel: function (vo) {
        ...
        if ((this.mainModel.vo.attr1 == 1 && this.mainModel.workPersonnel) ||
          this.mainModel.vo.status == 6) { //作业批准人签名
        ...
        workCardApi.tplDetail(_this.mainModel.vo.workPermit.cardTplId).then(function (model) {
          workCardApi.getWorkPermit(_this.mainModel.vo.workPermit.id).then(function (permitModel) {
            ...
            if (_this.seledTpl) {
              ...
              if (permitModel && permitModel.workTpl) model = permitModel.workTpl;
            } else {
              objModel.permitHandler(permitModel, model, false);
              // if(permitModel && permitModel.workTpl) model = permitModel.workTpl;
      ...
      selectedTplAddPtw: function (item) {
        ...
        workCardApi.tplDetail(item.tplId).then(function (data) {
          ...
          permitModel.gasDetectionRoleId = _model.gasDetectionRoleId;
          permitModel.enableGasDetection = _model.enableGasDetection;
          permitModel.enableGasDetectionSigner = _model.enableGasDetectionSigner;

          permitModel.enableCtrlMeasureRemark = _model.enableCtrlMeasureRemark;
          permitModel.enableCtrlMeasureSign = _model.enableCtrlMeasureSign;
          permitModel.enableCtrlMeasureGroup = _model.enableCtrlMeasureGroup;
          permitModel.enableCtrlMeasureVerifier = _model.enableCtrlMeasureVerifier;
          permitModel.enableUncheckedMeasureSign = _model.enableUncheckedMeasureSign;
    ...
    events: {
      "ev_doAddPtw": function (vo) {
        ...
        "ev_showAuthorize": function (vo) {
            // this.$refs.author._init(vo)
            this.authorizeModel.vo = vo;
        }
    },
    init: function () {
      api.__auth__ = {
