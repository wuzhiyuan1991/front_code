    ...
    return Vue.extend({
        ...
        methods: {
            initData:function(){
                ...
                _this.ppeList=m;
                if(this.model.enableUploadPicPpe==1) this.permitModel.enableUploadPicPpe=1;
                if(this.model.ppeCatalogSetting && !this.permitModel.ppeCatalogSetting && !this.permitModel.workTpl) this.permitModel.ppeCatalogSetting = this.model.ppeCatalogSetting;

                var list = [];
                var obj = null;
                _.each(this.ppeList, function (ppeListItem) {
                    _.each(ppeListItem.ppes, function (ppe) {
                        if(ppe.isExtra != '1')
                            list.push(ppe);
                        else obj = ppe;
                    })
