    ...
    return {
        ...
        cardTpl: function () {
            return {
                ...
                // 是否启用危害辨识 0 不启用 1 启用
                attr1: '0',
                gasDetectionRoleId: '0',
                enableGasDetection: '0',
                enableHazidSign: '0', //  危害辨识是否需要签字

                enableGasDetectionSigner: '0',  //气体检测 勾选
                enableCtrlMeasureRemark: '0',  // 安全措施是否启用核对项详细备注
                enableCtrlMeasureSign: '0',   // 安全措施是否启用核对项签名
                enableCtrlMeasureGroup: '0', // 安全措施是否启用核对项分组
                enableCtrlMeasureVerifier: '0', // 安全措施是否启用核对人
                enableUncheckedMeasureSign: '0', // 控制措施未勾选项是否需要签名

                applDepts: [], // 内部申请部门
                applContractors: [], // 承包商申请
                blindPlates: [], //盲板信息
                enableBlindPlatePulling: '0',
                enableBlindPlateStopping: '0',
                enableBlindPlate: '0'
            }
        },
        permitDetail: function () {
            return {
                fileList: [],
                blindPlates: [], // 盲板信息
                enableUploadPicPpe: '', // 个人防护是否需要上传图片 0:否 1：是
                attr3: null,
                id: null,
                //申请单位id
                applUnitId: null,
                ...
                //下面的内容在保存的时候去掉
                ppeVerifier: { id: "", name: "" },
                hazidVerifier: { id: "", name: "" },
                selworkPersonnels: { '1': [], '2': [], '3': [], '4': [], '5': [], '6': [], '11': [], '12': [], '13': [], '14': [], '15': {type:'15', users:[]} },
                //这里面存的是对象
        ...
        getItemId: function (arr) {
            var strArr = [];
            var str = '';
            strArr = _.pluck(arr, "id");
            if (strArr.length > 0) {
                str = strArr.join(',');
            }
            return str;
        ...
        toDeleteData: function (data) {
            for (var i = 0; i < data.jsonStrList.length; i++) {
                var item = data.jsonStrList[i];
                if (item.dataType == '1' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '2' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '3' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '4' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '5' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '6' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '7' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '8' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '9' && item.disable != '0') {
                    item.value = null;
                }
                else if (item.dataType == '10' && item.disable != '0') {
                    item.value = null
                }
                else if (item.dataType == '11' && item.disable != '0') {
                    item.value = null
                }


                // 固化字段校验
                if (item.code == 'applUnitId' && item.disable != '0') {
                    data.applUnitId = null
                }
                ...
                if (item.code == 'prodUnitId' && item.disable != '0') {
                    data.prodUnitId = null;
                }

                if (item.code == 'workPlace' && item.disable != '0') {
                    data.workPlace = null;
                }
                if (item.code == 'workEquipment' && item.disable != '0') {
                    data.workEquipment = null;
                }

                if (item.code == 'workContent' && item.disable != '0') {
                    data.workContent = null;
                }
                if (item.code == 'worker' && item.disable != '0') {
                    data.selworkPersonnels["4"] = [];
                    data.selworkPersonnels["5"] = [];
                }
                ...
                // 作业中所使用的主要工具/设备
                if (item.code == 'mainEquipment' && item.disable == '0') {
                    data.tempWorkStuffs.equipmentList = [];
                }
                // 特种作业人员/特种设备操作人员资格证
                if (item.code == 'specialWorker' && item.disable == '0') {
                    data.tempWorkStuffs.certificateList = [];
                }

                // 检维修人员
                if (item.code == 'maintainer' && item.disable != '0') {
                    data.selworkPersonnels["6"].splice(0);
                }

                // 工作安全分析
                if (item.code == 'jsaMasterId' && item.disable != '0') {
                    data.jsaMaster = [];
                }

                // 备注
                if (item.code == 'remark' && item.disable != '0') {
                    data.remark = null;
                }

                // 应急救援预案
                if (item.code == 'emerScheme' && item.disable != '0') {
                    data.schemeFiles = [];
                }

                // 作业计划书/方案
                if (item.code == 'workPlan' && item.disable != '0') {
                    data.planFiles = [];
                }

                // 附图
                if (item.code == 'attachedPicture' && item.disable != '0') {
                    data.figureFiles = [];
                }

                // 其他附件
                if (item.code == 'otherAttachment' && item.disable != '0') {
                    data.otherFiles = [];
                }
                ...
                //  作业方式
                if (item.code == 'operatingType' && item.disable != '0') {
                    data.workStuffs = _.reject(data.workStuffs, function (item) {
                        return item.type == '9';
                    });
                }
                // 作业监护人 tplModel.enableSupervisor=="1" &&
                if (item.code == 'supervisior' && item.disable != '0') {
                    data.selworkPersonnels["3"] = [];
                }
                // 实施安全教育人 tplModel.enableSafetyEducator=="1" &&
                if (item.code == 'safetyEducator' && item.disable != '0') {
                    data.selworkPersonnels["2"] = [];
                }
                // 工作安全分析
                if (item.code == 'jsaMasterId' && item.disable != '0') {
                    data.jsaMaster = null;
                    data.jsaMasterId = null;
                }
            }
        },
        ...
        getPermitDetail: function (data, model) {
            ...
            data.workIsolations = data.workIsolations.map(function (item) {
                return {
                    type: item.type,
                    isolatorId: _this.getItemId(item.isolators),
                    disisolatorId: _this.getItemId(item.disisolators)
                }
            });
            ...
            // 判断
            if (_.find(data.jsonStrList, function (item) {
                return item.code == 'safetyEducator' && item.disable == '0'
            })) {
                data.workPersonnels.push({ type: '2', users: [] });
            }
            if (_.find(data.jsonStrList, function (item) {
                return item.code == 'areaSupervisior' && item.disable == '0'
            })) {
                data.workPersonnels.push({ type: '15', users: [] });
            }
            if (_.find(data.jsonStrList, function (item) {
                return item.code == 'supervisior' && item.disable == '0'
            })) {
                data.workPersonnels.push({ type: '3', users: [] });
            }
            if (data.enableBlindPlateStopping == '0') { data.selworkPersonnels['11'] = []; data.selworkPersonnels['12'] = [] }
            if (data.enableBlindPlatePulling == '0') { data.selworkPersonnels['13'] = []; data.selworkPersonnels['14'] = [] }

            for (key in data.selworkPersonnels) {
                if(key != '15'){
                    data.selworkPersonnels[key].forEach(function (item) {
                        if (item.user && item.user.id) {
                            if (key == '2' || key == '3' ||key == '15') {
                                var obj = _.find(data.workPersonnels, function (person) {
                                    return key == person.type;
                                });
                                if (obj)
                                    obj.users.push({ id: item.user.id, name: item.user.name });
                            } else {
                    ...
                    var obj = _.find(data.workPersonnels, function (person) {
                        return '15' == person.type;
                    });
                    if (obj)
                        obj.users = [].concat(data.selworkPersonnels[key].users);
                }
            }
            // 许可签发人员添加
            if (data.signRoles && data.signRoles.length > 0) {
                data.signRoles.forEach(function (role) {
                    var obj = {
                        ...
                        attr2: role.id,
                        // id: role.id,
                        enableCtrlMeasureVerification: role.enableCtrlMeasureVerification,
                        enableCardModification: role.enableCardModification
                    }
                    if (data.workTpl && data.workTpl.id) {
                        obj.id = role.id;
                        obj.attr2 = role.attr2;
                    }
                    data.workPersonnels.push(obj);
                })
            }
            ...
            data.workStuffs = data.workStuffs.map(function (item) {
                ...
                if (item.type == '10' || item.type == '11') {
                    pms.workPersonnelId = item.workPersonnelId;
                }
                return pms;
            });
            // 10类型不提交
            data.workStuffs = _.reject(data.workStuffs, function (item) {
                return item.stuffType == '10' || item.stuffType == '11';
            });

            if (data.groupItemModel) {
                _.each(data.groupItemModel.list, function (group) {
                    if (group && group.id && group.children) {
                        var obj = _.find(data.workStuffs, function (item) {
                            return group.id == item.stuffId || item.id == group.id;
                        })
                        if (!data.workTpl || !data.workTpl.id) {
                            delete obj.id;
                        }

                        if (obj && group.children) {
                            var isTrue = _.find(group.children, function (child) {
                                return child.attr1 == '1'
                            });
                            if (isTrue) obj.attr1 = '1';
                        }
                        if (obj) {
                            obj.content = group.content;
                            obj.name = group.name;
                            obj.children = group.children.map(function (opt) {
                                return {
                                    stuffId: opt.stuffId,
                                    stuffType: opt.stuffType,
                                    ptwWorkVerifiers: opt.ptwWorkVerifiers,
                                    attr1: opt.attr1,
                                    content: opt.tempContent ? opt.tempContent : opt.name,
                                    checkResult: opt.checkResult,
                                    cloudFiles: opt.cloudFiles,
                                    isExtra: opt.isExtra,
                                    requireImg: opt.requireImg
                                }
                            });
                        }
                    }
            ...
            if (!data.attr4) { //allowStrEmpty 可参考 detailPanel 127行代码
                // 'criteria.' + type + 'Value.' + key + '_empty';
                if (!data.criteria) data.criteria = { strValue: {}, };
                data.criteria["strValue"]["attr4" + "_empty"] = "1";//str "1" int :1
            }
            if (!data.applUnitId) { //allowStrEmpty 可参考 detailPanel 127行代码
                // 'criteria.' + type + 'Value.' + key + '_empty';
                if (!data.criteria) data.criteria = { strValue: {}, };
                data.criteria["strValue"]["applUnitId" + "_empty"] = "1";//str "1" int :1
            }
            ...
            if (data.hazidVerifier) {
                data.hazidVerifierId = data.hazidVerifier.id;
            }
            delete data.ppeVerifier;
            delete data.hazidVerifier;
            delete data.groupItemModel;
        ...
        * */
        permitHandler: function (data, model, useModel, changeTpl) {
            if (!model) {
                _.extend(model, data.workTpl);
            }
            if (useModel) {
                data.workStuffs = model.ptwCardStuffs;
                data.gasDetectionRoleId = model.gasDetectionRoleId;
            }
            if (!changeTpl) {
                data.workPersonnels.forEach(function (person) {
                    if (["7", "1", "8", "9", "10"].indexOf(person.type) === -1) {//过滤掉特种人员
                        ...
                        if (person.type == "5") {
                            person.user = person.contractorEmp || person.user;
                        }
                        if (person.type == "2" || person.type == "3") {
                            _.each(person.users, function (user) {
                                data.selworkPersonnels[person.type].push({ user: { name: user.name, id: user.id } })
                            });
                        }else if(person.type == "15") {
                            b.users = [].concat(person.users);
                        } else {
                            if (person.user && person.user.name) person.name = person.user.name;
                            b.push(person);
                        }
                    }
                });
            ...
            data.workStuffs.forEach(function (workstuff) {
                ...
                } else if (workstuff.type == 10) {
                    if (!data.tempWorkStuffs.licenseList) data.tempWorkStuffs.licenseList = [];
                    data.tempWorkStuffs.licenseList.push(workstuff);
                } else if (workstuff.type == 11) {
                    if (!data.tempWorkStuffs.workCloseList) data.tempWorkStuffs.workCloseList = [];
                    data.tempWorkStuffs.workCloseList.push(workstuff);
                }
            });
            ...
            if (!data.hazidVerifier.id) {
                _.extend(data.hazidVerifier, {
                    id: LIB.user.id, name: LIB.user.name
                })
            }
            //处理作业关闭人员
        ...
        valiSavePerimit: function (data, tplModel, isSubmit) {
            ...
            //    return "请选择作业方式";
            // }
            if (data.enableBlindPlate == '1') {
                if (data.enableBlindPlatePulling == '0' && data.enableBlindPlateStopping == '0') {
                    return '请选择“堵”操作或者“抽”操作'
                };
                if (data.enableBlindPlateStopping == '1' && (data.selworkPersonnels['11'].length == 0 || data.selworkPersonnels['12'].length == 0)) {
                    if (data.selworkPersonnels['11'].length == 0) return '请选择“堵”操作作业人';
                    if (data.selworkPersonnels['12'].length == 0) return '请选择“堵”操作监护人';
                    return '请选择“堵”操作'
                }
                if (data.enableBlindPlatePulling == '1' && (data.selworkPersonnels['13'].length == 0 || data.selworkPersonnels['14'].length == 0)) {
                    if (data.selworkPersonnels['13'].length == 0) return '请选择“抽”操作作业人';
                    if (data.selworkPersonnels['14'].length == 0) return '请选择“抽”操作监护人';
                }
                if (!data.blindPlates || (data.blindPlates && data.blindPlates.length == 0)) {
                    return '请填写管道和盲板信息'
                }
            }
            ...
            for (var i = 0; i < data.jsonStrList.length; i++) {
                ...
                else if (item.dataType == '2' && item.isRequired == '1' && item.disable == '0') {
                    if (!(item.value == 0 || item.value)) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '3' && item.disable == '0') {
                    if ((item.value == 0 || item.value) && ((isNaN(item.value)) || item.value % 1 !== 0 || item.value < 0)) {
                        return item.name + " 为大于0的整数";
                    }
                }
                else if (item.dataType == '3' && item.isRequired == '1' && item.disable == '0') {
                    if (!(item.value == 0 || item.value)) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '4' && item.disable == '0') {
                    if ((item.value == 0 || item.value) && (isNaN(item.value) || item.value < 0)) {
                        return " " + item.name + " 为大于0的小数";
                    }
                }
                else if (item.dataType == '4' && item.isRequired == '1' && item.disable == '0') {
                    if (!(item.value == 0 || item.value)) {
                        return "请填写 " + item.name + " 为小数";
                    }
                }
                else if (item.dataType == '5' && item.isRequired == '1' && item.disable == '0') {
                    if (!item.value) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '6' && item.isRequired == '1' && item.disable == '0') {
                    if (!item.value || !item.value.value1 || !item.value.value2) {
                        return "请填写" + item.name;
                    }
                } else if (item.dataType == '6' && item.disable == '0') {
                    if (!item.value) continue;
                    if (item.value.value1 && item.value.value2) {
                        if (new Date(item.value.value2).getTime() <= (new Date(item.value.value1).getTime())) {
                            return item.name + '起始日期大于结束日期';
                        }
                    }
                }
                else if (item.dataType == '7' && item.isRequired == '1' && item.disable == '0') {
                    if (item.value && item.value.length == 0) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '8' && item.isRequired == '1' && item.disable == '0') {
                    if (item.value && item.value.length == 0) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '9' && item.isRequired == '1' && item.disable == '0') {
                    if (!item.value.value1 || !item.value.value2) {
                        return "请填写" + item.name;
                    }
                    if (item.value.value1.length == 0 || item.value.value2.length == 0) {
                        return "请填写" + item.name;
                    }
                }
                else if (item.dataType == '10' && item.isRequired == '1' && item.disable == '0') {
                    var opt = _.find(item.value, function (opt) {
                        return opt.result == '2';
                    });
                    if (!opt) return "请勾选" + item.name;
                }
                else if (item.dataType == '11' && item.isRequired == '1' && item.disable == '0') {
                    if (!item.value) {
                        return "请填写" + item.name;
                    }
                }
                ...
                // 固化字段校验
                if (!data.applUnitId && !data.attr4 && item.code == 'applUnitId' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择" + item.name;
                }
                if ((!data.workDepts || !data.workDepts.length) && (!data.workContractors || !data.workContractors.length) && item.code == 'workUnitId' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择" + item.name;
                ...
                {
                    // 校验长度
                    if (data.workDepts && data.workDepts.length && data.workDepts.length > 30 && item.code == 'workUnitId' && item.isRequired == '1' && item.disable == '0') {
                        return item.name + "请不要超过30个";
                    }
                    if (data.workContractors && data.workContractors.length && data.workContractors.length > 30 && item.code == 'workUnitId' && item.isRequired == '1' && item.disable == '0') {
                        return item.name + "请不要超过30个";
                    }
                }
                ...
                else if (data.workPlace && data.workPlace.length > 200 && item.code == 'workPlace' && item.disable == '0') {
                    return item.name + "不要超过200个字符";
                }

                if (!data.workEquipment && item.code == 'workEquipment' && item.isRequired == '1' && item.disable == '0') {
                    return "请填写" + item.name;
                }
                else if (item.code == 'workEquipment' && data.workEquipment && data.workEquipment.length > 500) {
                ...
                else if (item.code == 'workContent' && data.workContent && data.workContent.length > 500) {
                    return item.name + "不要超过500个字符";
                }
                ...
                if (data.remark && item.code == 'remark' && item.disable == '0' && data.remark.length > 500) {
                    return item.name + "不要超过500个字符";
                }
                ...
                if (item.code == 'permitTime') {
                    if (new Date(data.permitStartTime).getTime() <= new Date().getTime()) {
                        return item.name + '必须大于当前时间';
                    }
                    if (new Date(data.permitStartTime).getTime() >= new Date(data.permitEndTime).getTime()) {
                        return item.name + '结束日期必须大于起始日期';
                    }
                }
                ...
                //  作业方式
                if (tplModel.enableOperatingType == '1' && item.code == 'operatingType' && item.isRequired == '1' && item.disable == '0') {
                ...
                // 作业监护人 tplModel.enableSupervisor=="1" &&
                if (data.selworkPersonnels["3"].length === 0 && item.code == 'supervisior' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择" + item.name;
                }
                // 属地监护人 tplModel.enableSupervisor=="1" &&
                if (data.selworkPersonnels["15"].users.length === 0 && item.code == 'areaSupervisior' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择" + item.name;
                }
                // 实施安全教育人 tplModel.enableSafetyEducator=="1" &&
                if (data.selworkPersonnels["2"].length === 0 && item.code == 'safetyEducator' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择" + item.name;
                }
                // 工作安全分析
                if (data.pms.requiredJSA && !data.jsaMaster.id && item.code == 'jsaMasterId' && item.isRequired == '1' && item.disable == '0') {
                    return "请选择一个" + item.name;
                }
            }
        },
        valiSubmitPermit: function (data, tplModel) {
            var saveerror = this.valiSavePerimit(data, tplModel);
            ...
            if (data.enableProcessIsolation == 1) {
                ...
                if (!item || item.isolators.length == 0) {
                    return "请选择工艺隔离的隔离实施人"
                }
                if (!item.disisolators.length > 0) {
            ...
            if (data.enableGasDetection == 1 && data.enableGasDetectionSigner != 1) {
                if (!data.gasInspector || !data.gasInspector.id) {
                    return ' 请选择气体检测员'
                }
            }
        ...
        getViewData: function (permitModel, model) {
            ...
            users.forEach(function (item) {
                ...
                if(tempworkStuff)
                tempworkStuff.ptwWorkPersonnels = list[index] ? list[index].ptwWorkPersonnels : [];//这里使用同1个引用；
            })
        ...
        ptwSetting: getRootBusinessSet("ptw"),
        getSystembusinessset: function (name) {
            var setting = this.ptwSetting.children || [];
