define(function (require) {
  ...
  var detail = LIB.Vue.extend({
    ...
    methods: {
      ...
      isHasPrefix: function (tr) {
        if (tr.isGroup) {
          return false
        }
        console.log(tr.isGroup === undefined && !/^\d+.*/.test(tr.content))
        if (tr.isGroup === undefined && !/^\d+.*/.test(tr.content)) {
          return false
        }
        return true
      }
    ...
    events: {},
    filters: {
      sliceContentDigit: function (content, i, row, isPrefix) {
        try {
          console.log(row.isGroup, content, row, /^\d+.*/.test(content))
        } catch (error) {
          console.log(error)
        }
        if (row.isGroup || !/^\d+.*/.test(content)) {
          return content
        }
        var len = parseInt(content).toString().length
        return isPrefix ? content.slice(0, len + 1) : content.slice(len + 1)
      },
