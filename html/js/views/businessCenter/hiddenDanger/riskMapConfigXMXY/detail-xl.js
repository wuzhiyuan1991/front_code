define(function(require){
	...
	var dataModel = {
        ...
        tableModel: {
            tableObjectModel: {
                url: "tableobjectrel/checktable/list/{curPage}/{pageSize}",
                columns: [
                    {
                        title: "风险点名称",
                        width: 220,
		...
		methods:{
            ...
            doSaveCheckTable : function(selectedDatas) {
                ...
                if (selectedDatas) {
                    ...
                    api.createTableobjectrel(null, params).then(function (res) {
                        ...
                        _this.$refs.tableobjectTable.doQuery({checkObjectId : _this.mainModel.vo.id});
                    })
                }
            },
            ...
            doClickCOCell: function(data) {
                ...
                if ( 'isInherent' === cell.fieldName && isDisabled === false){
                    ...
                    api.updateTableobjectrel({id: row.id, isInherent: isInherent}).then(function () {
                        _this.$refs.tableobjectTable.doQuery({checkObjectId : _this.mainModel.vo.id});
                ...
                if ( 'isTemporary' === cell.fieldName && isDisabled === false){
                    ...
                    api.updateTableobjectrel({id: row.id, isTemporary: isTemporary,isTemporaryEnable: isTemporaryEnable}).then(function () {
                        _this.$refs.tableobjectTable.doQuery({checkObjectId : _this.mainModel.vo.id});
                ...
                if ( 'isTemporaryEnable' === cell.fieldName && isDisabled === false){
                    ...
                    api.updateTableobjectrel({id: row.id, isTemporaryEnable: isTemporaryEnable}).then(function () {
                        _this.$refs.tableobjectTable.doQuery({checkObjectId : _this.mainModel.vo.id});
                        // row.isTemporaryEnable = isTemporaryEnable;
                        LIB.Msg.info("操作成功");
                    })
            ...
            afterInitData : function() {
                this.$refs.tableobjectTable.doQuery({checkObjectId : this.mainModel.vo.id});
            },
            beforeInit : function() {
