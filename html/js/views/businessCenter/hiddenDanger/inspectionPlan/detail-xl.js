define(function (require) {
    ...
    var checkObjectSelect = require("./dialog/checkObject");
        ...
        tableModel: {
                ...
                columns: [{
                    ...
                    render: function (data) {
                        if (data.status == 2 || data.status == 3) {
                            return "<div style='color:#3c8dbc;cursor:pointer'>" + LIB.getDataDic("check_status", data.status) + "</div>"
                        }
                        return "<div >" + LIB.getDataDic("check_status", data.status) + "</div>"
                    },
                    width: 100
                }],
            ...
            userTableModel: {
                ...
                columns: [{
                    ...
                    _.extend(_.extend({}, LIB.tableMgr.column.company), {
                        filterType: null
                    }),
                    _.extend(_.extend({}, LIB.tableMgr.column.dept), {
                        filterType: null
                    }),
                    {
                        title: "手机",
                        ...
                        render: function (data) {
                            if (data && data.isValid == 1) {
                                return "<div style='color:red'>任务已失效</div>";
                            }
                            return '<a href="javascript:void(0);" class="is-valid-task">失效该任务</a>';
                        },
                        visible: false,
                        width: 120
                    }, {
    ...
    var detail = LIB.Vue.extend({
        ...
        watch: {
            ...
            'mainModel.vo.selectUser': function (val) {
                if(!val) {
                    val = 0;
                }
                if (val == '所有人') {
                    this.mainModel.vo.checkerId = null;
                    this.queryTaskData();
                } else {
                    this.doChooseUser(val)
                }
        ...
        methods: {
                        ...
                        if (!this.isTimeSettingAllowedToCrossAnother) {
                            ...
                            _.each(timeSettings, function (item) {
                                if (frequencyType == 1 && nextDayTimes.length > 0 &&
                                    !!_.find(nextDayTimes, function (data) {
                                        return item.start < data
                                    })) { //校验结束时间为次日时，是否比某条设置的开始时间大
                                    isCrossed = true;
                                    return;
                                }
                                if (!prevStart) {
            ...
            buildSaveData: function () {
                ...
                if (this.mainModel.bizType == 'investigate') {
                    resultDate.planCheckTableRel = {
                        checkTableId: _vo.checkTable.id,
                        checkObjId: ''
                    };
                }
                return resultDate;
            ...
            afterDoSave: function (opt, res) {
                ...
                var isInvalidColShow = this.hasAuth("edit") && (_vo.disable == "1") && (this.$route.query.bizType !== 'inspect');
                ...
                if (opt.type === "C") {
                    ...
                    api.getUsers({
                        ...
                        if (res.body.length > 0) { //判断是否有任务人的数据
                            ...
                            _this.mainModel.vo.checkerId = null;
                            // _this.mainModel.vo.checkerId = _this.inspectTaskModel.taskUsers[0].id;
            ...
            doShowCheckTaskFormModal4Update: function (param) {
                ...
                this.$refs.checktaskFormModal.init("update", {
                    id: this.mainModel.vo.id,
                    checkTaskId: param.entry.data.id
                });
            },
                ...
                if (selectedDatas) {
                    ...
                    api.saveUsers({
                        ...
                        api.getUsers({
                            ...
                            _this.mainModel.vo.selectUser = '所有人'
                            _this.mainModel.vo.checkerId = null;
                            // _this.mainModel.vo.checkerId = res.data[0].id;
                            // _this.mainModel.vo.checkerId = _this.inspectTaskModel.taskUsers[0].id;
                    ...
                    api.removeUsers({
                        ...
                        api.getUsers({
                            ...
                            if (res.body.length > 0) {
                                ...
                                // _this.mainModel.vo.checkerId = _this.mainModel.vo.selectUser;
                                // _this.inspectTaskModel.taskUser = res.data[0].id;
                                _this.inspectTaskModel.selectType = "1";
            ...
            doAppend: function () {
                if (this.mainModel.vo.disable == 1) {
                    ...
                    this.selectModel.userSelectModel.filterData = {
                        "criteria.strValue.excludeUserPlanId": this.mainModel.vo.id
                    };
                    var values = this.$refs.taskTable.values;
                    if (values && values.length == 1 && values[0].checkObjType == '4') {
                        this.selectModel.userSelectModel.filterData["criteria.strValue.labelId"] = values[0].checkObjId;
                        this.selectModel.userSelectModel.onlyType = 1;
                    } else {
                        this.selectModel.userSelectModel.onlyType = -1;
                    }
            ...
            selectPattern: function () {
                this.isTaskmodel = !!!this.isTaskmodel

                // this.isDefaultTaskPattern=!this.isDefaultTaskPattern

                if (this.isTaskmodel) {
                    this.doChooseUser(0)
                } else {
                    this.mainModel.vo.selectUser = '所有人'
                    this.mainModel.vo.checkerId = null;
                    this.queryTaskData();
                }
            },
            afterInitData: function () {
                var _vo = this.mainModel.vo;
                if (_vo.attr3 == '2') this.isDefaultTaskPattern = false;
                else this.isDefaultTaskPattern = true;
                this.mainModel.vo.planSetting.isWeekendInculed=='1'?this.frequencyModel.add.isWeekendInculedFlag = true:this.frequencyModel.add.isWeekendInculedFlag = false
                this.frequencyModel.add.isWeekendInculed = this.mainModel.vo.planSetting.isWeekendInculed;
            ...
            doTableCellClick: function (data) {
                ...
                if (data.cell.fieldName === "status") {
                    ...
                    if (checkerId == LIB.user.id && (status === '1' || (status === '4' && this.isLateCheckAllowed))) {
                        ...
                        var routerPart = "/html/main.html#!/hiddenDanger/businessCenter/checkRecord?method=check&checkTaskId=" + data.entry.data.groupId;
                        window.open(routerPart)
                    }
                    if (status === '2' || status === '3') {
                        window.isClickCheckTaskExecutBtn = true;
                        var routerPart = "/html/main.html#!/hiddenDanger/businessCenter/checkRecord?method=detail&id=" + data.entry.data.groupId;
                        window.open(routerPart)
            ...
            doAppendTask: function () {
                if (this.onlySetCheckObject) {
                    this.selectModel.objectSelectModel.visible = true;
                } else {
            ...
            doSaveTask: function (data) {
                ...
                var values = this.$refs.taskTable.values;
                if ((values && values.length == 1 && values[0].checkObjType == '4') || (data.checkObjType == '4' && values && values.length > 0)) {
                    this.selectModel.taskSelectModel.visible = false;
                    return;
                }

                var params = [];
                if (this.mainModel.bizType == 'investigate') { //巡检排查  已不使用
                    var obj = {
                        checkObjType: data.checkObjType,
                        checkObjId: data.checkObj.id,
                        checkTableId: data.checkTable.id
                    }
                    if ('1' === data.checkObjType) {
                        obj.dominationAreaId = data.checkObj.id;
                    } else {
                        obj.dominationAreaId = data.checkObj.dominationAreaId;
                    }
                    var params = [obj];
                } else {
                    if (data.checkTable && !this.onlySetCheckObject) {
                        for (var i = 0; i < data.checkTable.length; i++) {
                            for (var j = 0; j < data.checkObj.length; j++) {
                                var obj = {
                                    checkObjType: data.checkObjType,
                                    checkObjId: data.checkObj[j].id,
                                    checkTableId: data.checkTable[i].id
                                }
                                if ('1' === data.checkObjType) {
                                    obj.dominationAreaId = data.checkObj[j].id;
                                } else {
                                    obj.dominationAreaId = data.checkObj[j].dominationAreaId;
                                }
                                params.push(obj);
                            }
                        }
                    } else if (data.checkObj && this.onlySetCheckObject) {
                        for (var j = 0; j < data.checkObj.length; j++) {
                            var obj = {
                                checkObjType: '1',
                                checkObjId: data.checkObj[j].id,
                                checkTableId: "9999999999"
                            }
                            obj.dominationAreaId = data.checkObj[j].id;
                            params.push(obj);
                        }
                        this.selectModel.objectSelectModel.visible = false;
                    }
                }
                this.$api.saveTask({
                    id: this.mainModel.vo.id
            ...
            doUserInvalidClick: function (data) {
                ...
                if (target.classList.contains("is-valid-task")) {
                    ...
                    LIB.Modal.confirm({
                        title: '失效选中数据?',
                        onOk: function () {
                            api.invalidUserTask(null, {
                                checkerId: row.id,
                                checkPlanId: _this.mainModel.vo.id
                            }).then(function (res) {
                                LIB.Msg.info("已失效!");
                                _this.refreshTableData(_this.$refs.userTable);
                                _this.queryTaskData();
        ...
        ready: function () {
            ...
            //
            // var onlySetCheckObjectObj = companyBusinessSetState['checkTaskSet.onlySetCheckObject'];
            // this.onlySetCheckObject = onlySetCheckObjectObj == null ? false : onlySetCheckObjectObj.result === '2';

            this.enableCheckLevel = LIB.getBusinessSetByNamePath('common.enableCheckLevel').result === '2';
            this.onlySetCheckObject = LIB.getBusinessSetByNamePath('checkTaskSet.onlySetCheckObject').result === '2';
            if (this.onlySetCheckObject) {
                this.tableModel.taskTableModel.columns.splice(2, 1);
            }
        }
    });
