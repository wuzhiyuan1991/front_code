    ...
    var detail = LIB.Vue.extend({
        ...
        methods: {
            ...
            doShowSvRandomCheckRecordFormModal4Update: function (param) {
                ...
                this.$refs.svrandomcheckrecordFormModal.init("update", {id: this.mainModel.vo.id, svRandomCheckRecordId: param.entry.data.id},{id: this.mainModel.vo.id, svRandomCheckRecordId: param.entry.data.id});
            },
            doShowSvRandomCheckRecordFormModal4Create: function (param) {
                this.formModel.svRandomCheckRecordFormModel.show = true;
                this.$refs.svrandomcheckrecordFormModal.init("create", null, {compId: this.mainModel.vo.compId});
            },
            doSaveSvRandomCheckRecord: function (data) {
                var _this = this;
                var params = _.cloneDeep(data);
                if (data) {
                    if (params.checkDate && params.checkDate.length === 10) {
                        params.checkDate += ' 00:00:00'
                    }
                    api.saveSvRandomCheckRecord({id: this.mainModel.vo.id}, params).then(function () {
            ...
            doUpdateSvRandomCheckRecord: function (data) {
                ...
                if (data) {
                    ...
                    api.updateSvRandomCheckRecord({id: this.mainModel.vo.id}, params).then(function () {
                        _this.refreshTableData(_this.$refs.svrandomcheckrecordTable);
                    });
                }
            },
            doRemoveSvRandomCheckRecords: function (item) {
                ...
                LIB.Modal.confirm({
                    title: '确定删除数据?',
                    onOk: function() {
                        api.removeSvRandomCheckRecords({id: _this.mainModel.vo.id}, [{id: data.id}]).then(function () {
