	...
	var detail = LIB.Vue.extend({
		...
		methods:{
			...
			doExportPdf: function () {
				window.open("/checktaskgroup/" + this.mainModel.vo.id + "/exportPdf/3" );

			},
            getBusinessSetStateByNamePath: function (val) {
				return LIB.getBusinessSetStateByNamePath(val);
            },
			...
			doMakeCheckRecord : function() {
				if(!this.isShowExcute){
                    LIB.Msg.warning("当前任务只能在app里面执行");
                    return ;
                }
				if(this.mainModel.vo.checkerId && this.mainModel.vo.checkerId != LIB.user.id) {
                	LIB.Msg.warning("只能执行自己的任务");
                    return;
                }else if(this.mainModel.vo.checkerIds && this.mainModel.vo.checkerIds.indexOf(LIB.user.id) == -1){
					LIB.Msg.warning("只能执行自己的任务");
					return;
				}
				...
				if(this.mainModel.bizType) {
					routerPart = "/"+startRoute+"/businessCenter/checkRecord?bizType="+this.mainModel.bizType+"&keepUrlParam=true&method=check&checkTaskId="+_this.mainModel.vo.id;
				}
				this.$router.go(routerPart);
			},
			afterInitData: function () {
				if(this.mainModel.vo.checkerNames && this.mainModel.vo.checkerNames.toString().indexOf(",")>-1)
                	this.mainModel.vo.checkerNames = _.union(this.mainModel.vo.checkerNames.toString().split(",")).join("，");
