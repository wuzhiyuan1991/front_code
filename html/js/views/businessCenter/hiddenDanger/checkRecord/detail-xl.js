define(function (require) {
  ...
  var checkObjectSelect = require("./dialog/checkObjectSelect");
  ...
  var dataModel = {
      ...
      },
      {
        ...
        render: function (data) {
          ...
          if (data.failedNum && data.failedNum > 1) {
            str = "<a href='javascript:void(0);' style='color:red;display: flex;align-items: center;'>不合格" + "<span style='border-radius: 50%;background: red;color:#fff;display: inline-block;width:15px;height: 15px;font-size: 12px;text-align: center;margin-left: 5px;'>" + data.failedNum + "</span></a>";
          }
        ...
        },
        {
          ...
          render: function (data) {
            ...
            if (isDone) {
              ...
              if (userId === data.checkerId || (data.checkerIds && data.checkerIds.includes(userId))) {
    ...
    onlySetCheckObject: false,
    checkTableList: [],
    checkTableIndex: 0,
    checkRecordDetail: null,
  };
  ...
  var detail = LIB.Vue.extend({
    ...
    computed: {
      ...
      checkObjModelItems: function () {
        var isFilters = []
        var _this = this;
        var list = [];
        var result = 0;
        // 搜索进行过滤
        if (this.mainModel.groupSearchContent) {
          isFilters = this.filterTaskGroup(this.checkObjModel.items, this.mainModel.groupSearchContent)
        }
        if (this.itemSelectModel[0]) {
          result += 1;
        }
        if (this.itemSelectModel[1]) {
          result += 10;
        }
        if (this.itemSelectModel[2]) {
          result += 100;
        }
        if (result == 0) return list;
        // if(this.itemSelectModel[0]){
        var items = isFilters.length ? isFilters : this.checkObjModel.items
        for (var i = 0; i < items.length; i++) {
          var itemList = [];
          var group = items[i];
          itemList = itemList.concat(_.filter(group.itemList, function (item) {
            if (result == 1) {
              ...
              return item.checkResult == '1' || item.checkResult == '0';
            } else if (result == 110) {
              return item.checkResult == '2' || item.checkResult == '0';
            } else if (result == 101) {
              return item.checkResult == '1' || item.checkResult == '2';
            } else {
              return item;
            }
          }));
          var checkStartDate = group.itemList.length ? group.itemList[0].checkStartDate : null;
          ...
          if (itemList && itemList.length > 0) {
            var obj = {
              ...
              groupOrderNo: group.groupOrderNo,
              itemList: itemList
            };
            if (checkStartDate) {
              obj.checkStartDate = checkStartDate;
            }
            list.push(obj);
          }
        }
        // }
        // if(this.itemSelectModel[1]){
        //     for (var i=0; i<this.checkObjModel.items.length; i++) {
        //         var itemList = [];
        //
        //         itemList = itemList.concat(_.filter(this.checkObjModel.items[i].itemList, function (item) {
        //             return item.checkResult == '0';
        //         }));
        //         if(itemList && itemList.length>0){
        //             var obj = {
        //                 groupName:this.checkObjModel.items[i].groupName,
        //                 groupOrderNo:this.checkObjModel.items[i].groupOrderNo,
        //                 itemList:itemList
        //             };
        //             list.push(obj);
        //         }
    ...
    methods: {
      // doChangeItems:function (val) {
      //     debugger
      //     console.log(this.itemSelectModel[val]);
      //     this.itemSelectModel[val] = !this.itemSelectModel[val];
      // },
      onTabSelected: function (index, item) {
        var checkTableId = item.id;
        var data = _.filter(this.checkRecordDetail, function (item) {
          return item.checkTable.id === checkTableId;
        });
        this.checkObjModel.items = _.propertyOf(data[0])("checkRecordDetailVoList") || [];
        var checkList = [];
        data[0].checkRecordDetailVoList.forEach(function (item) {
      ...
      doTrajectoryMap: function () {
        window.open(window.location.origin + "/html/trajectorymap.html?id=" + this._checkObj.checkTaskId);
      },
      newVO: newVO,
      ...
      onEditCheckResult: function (vo, fieldName, str) {
        ...
        this.$broadcast(
          ...
          fieldName,
          str,
          this.checkObjModel.checkObjects[0].checkObjectTableBean.dominationAreaId
        );
      },
      ...
      doShowCheckTaskSelectModal: function () {
        ...
        this.selectModal.checkTaskSelectModal.filterData.bizType = this.mainModel.bizType;
      },
      // 选择检查任务弹窗保存检查任务
      doSaveCheckTask: function (items) {
        ...
        if (_data) {
          ...
          _vo.checkTask = {
            id: _data.id,
            name: _data.groupName + "#" + _data.num
          };
          ...
          _vo.checkLevel = _data.checkLevel;
      ...
      changeIconColor: function (id, hasContent, length) {
        ...
        var objArr = this.checkObjModel.items;
        ...
        //edit litao 日期  这里重新复制会导致items 父类div 重新 重绘制， 以至于滚动条归0
      ...
      _getCheckTaskConfig: function (isLateCheckAllowed) {
        var _this = this;
        _this.selectModal.checkTaskSelectModal.filterData["criteria.intValue"].isLateCheckAllowed = (isLateCheckAllowed.result === '1' ? 0 : 1);
      },
      ...
      doTaskClick: function (data) {
        ...
        if (target.classList.contains("check-task-do")) {
          ...
          this.getEnvConfig(row.checkTableId, row.id, row.checkerId, row.checkPlanId);
          this._checkObj = {
          ...
          this._checkObj.checkTaskId = row.id;
      ...
      _queryDoneCheckTable: function (checkTableId, id) {
        var _this = this;
        if (_this.onlySetCheckObject) {
          api.getCheckRecordDetail({
            checkTaskId: id
          }).then(function (res) {
            _this.checkRecordDetail = res.data;
            _this.checkTableList = [];
            _this.checkTableIndex = 0;
            _this.checkObjModel.items = [];
            _this.hasTrajectoryMap = false;
            if (res.data) {
              _this.mainModel.vo.collaborators = res.data[0].collaborators;

              res.data.forEach(function (item) {
                _this.checkTableList.push(item.checkTable);
              });
              _this.checkObjModel.items = _.propertyOf(res.data[0])("checkRecordDetailVoList") || [];
              var checkList = [];
              res.data[0].checkRecordDetailVoList.forEach(function (item) {
                checkList = checkList.concat(item.itemList);
              });
              _this.hasTrajectoryMap = checkList.some(function (item) {
                return 'location' in item;
              });
            }
          })
        } else { // checkObjModel
          api.getDoneCheckTable({
            checkTaskId: id
          }).then(function (res) {

            _this.mainModel.vo.collaborators = [];
            _this.checkObjModel.items = [];
            if (!res.data) return;

            _this.mainModel.vo.collaborators = res.data.collaborators;

            _this.checkObjModel.items = _.propertyOf(res.data)("checkRecordDetailVoList") || [];
            var checkList = [];
            res.data.checkRecordDetailVoList.forEach(function (item) {
              checkList = checkList.concat(item.itemList);
            });
            _this.hasTrajectoryMap = checkList.some(function (item) {
              return 'location' in item;
       ...
       * @param checkTaskId
       */
      queryCheckItem: function (id, checkTaskId, checkerId, checkPlanId) {
        ...
        api.getCheckTable({
          ...
          if (LIB.getBusinessSetStateByNamePath("checkSubmit.isCheckTableSetDefaultResult")) { //检查表单独设置默认结果
            var defaultResult = data.data.defaultResult;
            if (defaultResult) { //空值或‘3’=无
              if (defaultResult === '0' || defaultResult === '1') { //合格||不合格
                _this.envConfig.defaultValue = defaultResult;
              } else if (defaultResult === '2') { //不涉及
                if (_this.envConfig.notRefer.result === '3') { //存在不涉及列
                  _this.envConfig.defaultValue = defaultResult;
                } else {
                  _this.envConfig.defaultValue = null;
                }
              } else if (defaultResult === '3') { //无
                _this.envConfig.defaultValue = null;
              }
            }
          }
          ...
          _.each(_list, function (checkItemGroup) {
            _.each(checkItemGroup.itemList, function (checkItem) {
              ...
              if (checkItem.attr2 && checkItem.attr2 == '1') {
                checkRecord["checkObjId"] = checkItem.equipId;
                checkRecord['checkObjType'] = 2;
              } else {
       ...
       * @param checkTaskId
       */
      getEnvConfig: function (checkTableId, checkTaskId, checkerId, checkPlanId) {
        var _this = this;
        ...
        api.getEnvConfig({
          ...
          _this.queryCheckItem(checkTableId, checkTaskId, checkerId, checkPlanId);
      ...
      getCheckObjects: function (id) {
        ...
        api.getCheckGroupTask({
          id: id
        }).then(function (res) {
          var items = res.data;
          // console.log(res.da)
          // _this.checkObjModel.items = [];
          // _this.$refs.checktaskTable.doClearData();
          if (items.length > 0) {
            ...
            if (_this.isTaskDone) {
              ...
              _this.getEnvConfig(items[0].checkTableId, items[0].id, LIB.user.id, items[0].checkPlanId, items[0].checkPlanId);
            }
          }
          // 用于给检查项的检查对象赋值
          _this._checkObj = {
            ...
            checkTaskId: items[0].id,
          };
          // console.log(items)
          _this.checkObjModel.checkObjects = items;

          // this.hasTrajectorymap= this.checkObjModel.some(function ( item) {
          //     return 'location' in item;
          // })
        })
      },
      ...
      afterInit: function () {
        ...
        // this.itemSelectModel[0] = true;
        // this.itemSelectModel[1] = true;
        // this.itemSelectModel[2] = false;
        ...
        if (this.mainModel.opType === "create") {
          ...
          api.getCheckTaskGroup({
            ...
            _vo.checkerId = data.checkerId == undefined ? LIB.user.id : data.checkerId;
            _vo.checkTaskId = data.id;
            _vo.startDate = data.startDate;
            _vo.endDate = data.endDate;
            _vo.orgId = data.orgId;
            _vo.checkLevel = data.checkLevel;
      ...
      afterInitData: function () {
        ...
        this.mainModel.vo.checkDate = (new Date()).Format("yyyy-MM-dd hh:mm:ss");
        // api.getCheckRecords({id:this.mainModel.vo.id}).then(function (res) {
        //     //协同检查人
        // });
        var columns = this.itemColumns;
        if (this.mainModel.vo.checkPlan.checkTables.length==0) {
        ...
        if (this.mainModel.vo.checkPlan.checkTables[0].bizTypeSc1 == 'isp_route') {//巡检模式
          _.each(columns, function (item) {
            if (item.fieldName == 'checkObjName' || item.fieldName == 'checkStd') {
                item.visible = true;
            }
      ...
      beforeDoSave: function () {
        ...
        _.each(this._saveData, function (item, index) {
          if (item && item.checkTableId === '9999999999') {
            isCheckResultValid = false;
            detailErrorMsg = "该任务暂不支持网页端执行";
          }

        })
        if (!isCheckResultValid) {
          LIB.Msg.error(detailErrorMsg, 5);
          return false;
        }
        // 循环数据
        _.each(this._saveData, function (item, index) {
      ...
      filterTaskGroup: function (checkObjModelItems, inputValue) {
        var _this = this
        // 空字符恢复
        inputValue = inputValue.trim()
        if (!inputValue && !inputValue.length) {
          return checkObjModelItems
        }
        var checkRecordDetailVoList = []
        checkObjModelItems.forEach(function (group) {
          var backList = []
          var updateGroup = {}
        ...
        return checkRecordDetailVoList
      },
      doUpdateSearchValue: function (value) {
        this.mainModel.groupSearchContent = value
      }
    },
    events: {
      "ev_gridRefresh": function (id, checkResult, problem, remark, checkItemId, latentDefect, type, hasContent, legalRegulationId, groupId, checkRecordDetailProblems) {
        ...
        var length = 0;
        if (checkRecordDetailProblems && checkRecordDetailProblems.length > 0) {
          var length = checkRecordDetailProblems.length;
        }

        this.changeIconColor(checkItemId, hasContent, length);
    ...
    ready: function () {
      ...
      var isLateCheckAllowed = companyBusinessSetState["checkTaskSet.isLateCheckAllowed"];
      this._getCheckTaskConfig(isLateCheckAllowed);

      var isShowVetoItem = companyBusinessSetState["checkSubmit.isShowVetoItem"];
      this.getShowVetoItemSetting(isShowVetoItem);

      this.enableCheckLevel = companyBusinessSetState["common.enableCheckLevel"].result === '2';
      this.enableTrajectoryMap = companyBusinessSetState["common.checkRecordShowLocation"].result === '2';
      var onlySetCheckObject = companyBusinessSetState["checkTaskSet.onlySetCheckObject"];
      if (onlySetCheckObject && onlySetCheckObject.result === '2') {
        this.taskTable.columns.splice(2, 1);
        this.onlySetCheckObject = true;
      }
