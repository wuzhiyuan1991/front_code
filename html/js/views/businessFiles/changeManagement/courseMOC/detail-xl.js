define(function (require) {
    ...
    var dataModel = {
        ...
        importProgress: {
            show: false,
            uploadUrl: '/changemanagement/{id}/importResult',
            downloadUrl: '/cultivaterecord/importExcelTpl/down'
        }
    };
    //Vue组件
	/**
    ...
    var detail = LIB.Vue.extend({
        ...
        methods: {
            ...
            dopass: function () {
                var _this = this
                api.update(null, { id: this.mainModel.vo.id, status: 5 }).then(function () {
                    LIB.Msg.success('提交验收成功')
                    _this.$dispatch("ev_dtCreate");
                    _this.doClose()
                })
            },
            doImport: function () {
                this.importProgress.uploadUrl='/changemanagement/'+this.mainModel.vo.id+'/importResult'
            ...
            doRemovepxjl: function (item) {
                ...
                LIB.Modal.confirm({
                    title: '删除当前数据?',
                    onOk: function () {
                        api.removeCultivateRecords({ id: _this.mainModel.vo.id }, [{ id: data.id }]).then(function () {
                            LIB.Msg.info('删除成功')
                            _this.$refs.pxjl.doQuery({ id: _this.mainModel.vo.id })
                        });
                    }
                });
            },
            ...
            doSaveItem: function (data, title) {
                var _this = this
                if (title == '新增') {
                    api.saveCultivateRecord({ id: this.mainModel.vo.id }, data).then(function (res) {
                        LIB.Msg.info('新增成功')
                        _this.$refs.pxjl.doQuery({ id: _this.mainModel.vo.id })
                    })
                } else {
                    api.updateCultivateRecord({ id: this.mainModel.vo.id }, data).then(function (res) {
        ...
        events: {
            "ev_upload_success": function () {
                ...
                this.$refs.pxjl.doQuery({ id: this.mainModel.vo.id })
            }
        },
        init: function () {
            this.$api = api;
