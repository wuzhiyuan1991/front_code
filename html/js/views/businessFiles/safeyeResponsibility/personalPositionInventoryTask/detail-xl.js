define(function(require){
	...
	var detail = LIB.Vue.extend({
		...
		methods:{
            ...
            doExecute: function () {
                var _this = this;
                api.getTaskList({positionInventoryId: this.mainModel.vo.positionInventoryId, frequencyType: this.mainModel.vo.frequencyType}).then(function (res) {
            ...
            _normalizeGroupData: function (data) {
                ...
                var items = _.map(data.item, function (item) {
                    return {
                        positionInventoryId: item.positionInventoryId,
                        positionInventoryTaskId: taskId,
                        content: item.content,
                        groupId: item.groupId,
                        groupName: _.get(item, "positionInventoryGroup.name"),
                        groupOrderNo: _.get(item, "positionInventoryGroup.orderNo"),
                        id: item.id,
                        orderNo: item.orderNo,
                        result: item.result,
                        score: item.score,
            ...
            doSave: function () {
                ...
                if (someUnCheck) {
                    return LIB.Msg.error("有工作项未填写实际得分");
                }
                var taskId = this.mainModel.vo.id;
                var params = {
                    positionInventoryId: this.mainModel.vo.positionInventoryId,
                    positionInventoryTaskId: taskId,
                    ...
                    positionInventoryRecordDetails: _.map(items, function (item) {
                        return {
                            id: item.recordId,
                            positionInventoryId: item.positionInventoryId,
                            positionInventoryTaskId: item.positionInventoryTaskId,
