define(function (require) {
	...
	var lawsContentFormModal = require("componentsEx/formModal/lawsContentFormModal");
	...
	var dataModel = {
		...
		mainModel: {
			...
			rules: {
				...
				"name": [LIB.formRuleMgr.require("法规名称"), LIB.formRuleMgr.length(100)],
				"disable": LIB.formRuleMgr.require("状态"),
				"annulment": [LIB.formRuleMgr.length(500)],
				"annulmentDate": [LIB.formRuleMgr.allowStrEmpty],
				"effectiveDate": [LIB.formRuleMgr.require("实施日期"), LIB.formRuleMgr.allowStrEmpty],
				"effectiveLevel": [LIB.formRuleMgr.require("效力级别"), LIB.formRuleMgr.allowIntEmpty].concat(LIB.formRuleMgr.allowIntEmpty),
				"isRevise": [LIB.formRuleMgr.allowIntEmpty].concat(LIB.formRuleMgr.allowIntEmpty),
				"issuedNumber": [LIB.formRuleMgr.length(200)],
				"limitation": [LIB.formRuleMgr.require("时效性"), LIB.formRuleMgr.allowIntEmpty].concat(LIB.formRuleMgr.allowIntEmpty),
				"publishAuthority": [LIB.formRuleMgr.require("发布机关"), LIB.formRuleMgr.length(200)],
				"publishDate": [LIB.formRuleMgr.require("发布日期"), LIB.formRuleMgr.allowStrEmpty],
				"remark": [LIB.formRuleMgr.length(500)],
				"resume": [LIB.formRuleMgr.length(500)],
				"revise": [LIB.formRuleMgr.length(500)],
				'typeId': [LIB.formRuleMgr.require("法规分类")]
			}
		},
		...
		tableModel: {
			lawsContentTableModel: LIB.Opts.extendDetailTableOpt({
				url: "lawschapter/lawscontents/list/{curPage}/{pageSize}?criteria.orderValue.fieldName=orderNo&criteria.orderValue.orderType=0",
				columns: [
		...
		formModel: {
			lawsContentFormModel: {
				show: false,
				hiddenFields: ["lawsId"],
				queryUrl: "lawschapter/{id}/lawscontent/{lawscontentId}"
			},
		},
		selectModel: {
		},
	...
	var detail = LIB.Vue.extend({
		...
		,
		components: {
			"lawsContentFormModal": lawsContentFormModal,
		...
		methods: {
				...
				this.$refs.ruleform.validate(function (valid) {
					if (valid) {
						if (_this.typeSelectData.length > 0) {
							_this.mainModel.vo.lawsType = _this.typeSelectData[0]
						}
						api.updateRevision(_this.mainModel.vo).then(function (res) {
							LIB.Msg.success("保存成功");
							api.querylawsrevise({ 'discernId': _this.mainModel.vo.discernId }).then(function (res) {//获取新的修订记录

								var newResive = 0
								for (var index = 0; index < res.data.length; index++) {
									var isNew = true
									for (var j = 0; j < _this.reviseList.length; j++) {
										if (res.data[index].lawsId == _this.reviseList[j].lawsId) {
											isNew = false
											break
										}
									}
								...
								_this.selectRevise(_this.revise.lawsId)
			...
			doReviseCancel: function () {
				...
				var data = _.filter(that.reviseList, function (item) {
					return item.lawsId == that.mainModel.vo.id
				})
				if (data.length > 0) {
			...
			doDeleteType: function () {
				...
				LIB.Modal.confirm({
					...
					onOk: function () {
						api.removeLawsChapters({ id: _this.mainModel.vo.id }, [{ id: id }]).then(function () {
							LIB.Msg.success("删除成功");
							_this.$api.queryLawsChapters({ 'id': _this.mainModel.vo.id, "criteria.orderValue.fieldName": "orderNo", "criteria.orderValue.orderType": 0 }).then(function (res) {
								...
								if (typeId) {
									...
									_this.$refs.lawsContentTable.doCleanRefresh(params);
								} else {
									_this.$refs.lawsContentTable.doClearData();
								}
							})
			...
			doTreeNodeClick: function () {
				...
				if (typeId) {
					...
					this.$refs.lawsContentTable.doCleanRefresh(params);
				} else {
					this.$refs.lawsContentTable.doClearData();
				}

			},
			_getTreeList: function () {
				var that = this;
				this.$api.queryLawsChapters({ 'id': that.mainModel.vo.id, "criteria.orderValue.fieldName": "orderNo", "criteria.orderValue.orderType": 0 }).then(function (res) {
					that.chapterTypes = res.data.list
					that.treeSelectData = res.data.list.slice(0, 1);
				})
			},
			// beforeDoSave: function () {
			// 	// this.mainModel.vo.typeId = this.legalRegulationType.id
			// },
			newVO: newVO,
			doRevise: function () {
				...
				_.each(Object.keys(this.mainModel.vo), function (key) {
					if (key !== 'effectiveLevel' && key !== 'name' && key !== 'publishAuthority' && key !== 'revise' && key !== 'lawsType' && key !== 'typeId' && key !== 'discernId' && key !== 'id') {
			...
			doShowLawsContentFormModal4Update: function (param) {
				this.formModel.lawsContentFormModel.show = true;
				this.$refs.lawsContentFormModal.init("update", { id: this.mainModel.vo.id, lawscontentId: param.entry.data.id });
			},
			doShowLawsContentFormModal4Create: function (param) {
				var typeId = _.get(this.treeSelectData, "[0].id");
				if (!typeId) {
					LIB.Msg.error("请选择分类");
					return;
				...
				this.formModel.lawsContentFormModel.show = true;
				this.$refs.lawsContentFormModal.init("create");
			},
			doSaveLawsContent: function (data) {
				if (data) {
					var _this = this;
					data.lawsId = this.mainModel.vo.id;
					api.saveContent({ id: data.lawsChapter.id }, data).then(function () {
						_this.$refs.lawsContentTable.doQuery({ id: data.lawsChapter.id });
						// _this.refreshTableData(_this.$refs.lawsContentTable);
					});
				}
			},
			doUpdateLawsContent: function (data) {
				if (data) {
					var _this = this;
					api.updateContent({ id: data.lawsChapter.id }, data).then(function () {
						_this.$refs.lawsContentTable.doQuery({ id: data.lawsChapter.id });
						// _this.refreshTableData(_this.$refs.lawsContentTable);
					});
				}
			},
			doRemoveLawsContent: function (item) {
				var _this = this;
			...
			afterInitData: function () {
				...
				if (this.mainModel.opType !== 'create') {
					LIB.globalLoader.show()
					this.$api.querylawsrevise({ 'discernId': that.mainModel.vo.discernId }).then(function (res) {
					...
					this.$api.queryLawsChapters({ 'id': that.mainModel.vo.id, "criteria.orderValue.fieldName": "orderNo", "criteria.orderValue.orderType": 0 }).then(function (res) {
						...
						if (that.chapterTypes.length > 0) {
							...
							that.$refs.lawsContentTable.doQuery({ id: that.treeSelectData[0].id });
						} else {
							that.treeSelectData = []
						}
					})
			...
			afterInit: function () {
				...
				if (this.mainModel.opType == 'create') {
					...
					// 	})
					// 	_this.mainModel.vo.typeId = _this.typeid
					// })
				}
