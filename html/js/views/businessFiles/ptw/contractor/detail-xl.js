	...
	var dataModel = {
		...
		mainModel : {
	        ...
	        rules:{
				...
				"businessScope" : [LIB.formRuleMgr.length(200)],
		...
		fileModel: {
            default: {
                cfg: {
                    ...
                    filters: {
                        max_file_size: '50mb',
                        mime_types: [{title: "file", extensions: "pdf,doc,docx,xls,xlsx,ppt,pptx,word,jpg,jpeg,png"}]
                    }
                },
                data: []
            },
			other: {
				cfg: {
					...
					filters: {
						max_file_size: '50mb',
						mime_types: [{title: "file", extensions: "png,jpg,jepg,pdf,doc,docx,xls,xlsx,ppt,pptx,word"}]
					}
				},
				data: []
			}
...
*/
	...
	var detail = LIB.Vue.extend({
		...
		methods:{
			newVO : newVO,
			afterInitData: function(items){
                var files = [];
                _.forEach(items, function (item) {
                    if(item.dataType === 'CBS2') {
                        files.push({
                            fileId: item.id,
                            fileExt: item.ext,
			...
			changecontractorType:function(val){
				if (val==1||val==2) {
					this.contractorType=val
					$('.simple-card-input-box').hide()
				}else{
					this.contractorType=val
					$('.simple-card-input-box').show()
				}
			},
			...
			doUpdatePassWord:function () {
				var contractorEmpTableModel = this.tableModel.contractorEmpTableModel;debugger
				if(contractorEmpTableModel.cureObj && contractorEmpTableModel.cureObj.id && contractorEmpTableModel.cureObj.telephone){
					this.resetPsd.show = true;
					this.resetPsd.title = "重置密码";
					this.resetPsd.id = contractorEmpTableModel.cureObj.id;
					this.$broadcast('ev_editReload_psd', contractorEmpTableModel.cureObj.telephone);
				}
			},
