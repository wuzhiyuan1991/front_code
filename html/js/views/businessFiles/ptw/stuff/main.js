    ...
    var vm = LIB.VueEx.extend({
        ...
        methods: {
			...
			workCatalogInit:function(workCatalogId,type, isSelect){
				...
				if(!workCatalogId) return ;
				var pms={
					'workCatalog.id':workCatalogId,
					type:type,
					compId: _this.selectOrgId,
				...
				this.$api.getCatalogDetail(pms).then(function (data) {
                    ...
                    if(isSelect) _this.$refs.workContent.init(workCatalogId,_this.selectOrgId);
				})
			},
			...
			initFun: function (data) {
				this.stuffTypes = LIB.getDataDicList("iptw_stuff_type");
				if(!data[0]) {
					_.each(this.stuffTypes, function (item) {
						item = [];
                    });
					this.workCatalogs = [];
					return ;
                ...
                _this.workCatalogInit(first.id, type, true);
            ...
            doOrgCategoryChange: function(obj) {
                ...
                this.getCatalogList();
                // 更新列表操作
                this.$broadcast('do-org-category-change', obj.nodeId);
            },
            // doOrgCategoryChange: function (obj) {
			...
			getCatalogList: function () {
                var _this=this;
                var obj = null;
				obj = {compId: this.selectOrgId};
                this.$api.getWorkCatalogs(obj).then(function (data) {
		...
		ready: function () {
            ...
            this.$broadcast('do-org-category-change',  LIB.user.compId);
