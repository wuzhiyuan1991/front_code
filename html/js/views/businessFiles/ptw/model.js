    ...
    return {
        ...
        cardTpl:function() {
            return {
                ...
                workLevelId:"",
                attr1:'0',
                enableUploadPicPpe: '0' ,// 个人防护 需要在核对中添加 附录照片功能  0: 否  1: 是
                gasDetectionRoleId: '', //气体检测 签发人员
                gasDetectionRole:{id:null},
                ...
                // gasDetectionPersonnelId: null
                expireNoticeRoles :null,
                expireNoticeTime :60,
                enableExpireNotice: '0'
            }
        },
        ...
        saveHandleTpl:function (data) {
            data.ptwCardStuffs = data.ptwCardStuffs.map(function (item) {
                ...
                if(item.type == '10'){
                    pms['cardSignRoleId'] = item.cardSignRoleId || null
                }
                if(item.type == '11'){
                    pms['cardSignRoleId'] = item.cardSignRoleId || null
                }
                if(item.type == '4' && !item.gasType && item.children) {
                    pms['children'] = item.children.map(function (opt) {
                        return {
                            // id: opt.id,
                            groupId:item.id,
                            stuffType:'4',
                            ...
                            isExtra:opt.isExtra || '0',
                            ptwCardVerifiers:_.map(opt.ptwCardVerifiers, function (verifier) {
                                return {
                                    // id: verifier.id,
                                    name: data.enableCtrlMeasureVerifier=='1'? verifier.name: ''
                                }
                            }) || []
                        }
                    }) || []
            ...
            data.workCatalog ={id: data.workCatalog.id}
