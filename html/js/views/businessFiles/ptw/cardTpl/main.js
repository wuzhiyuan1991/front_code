define(function (require) {
	...
	var detailPanel = require("./detail");
 	...
 	var copyFormModal = require("./copyFormModal");
...
//		mixins : [LIB.VueMixin.dataDic, LIB.VueMixin.auth, LIB.VueMixin.mainLegacyPanel],
        ...
        methods: {
			...
			doCopyShow: function () {
                var item = this.tableModel.selectedDatas[0];
                if(item) this.showFormModal(item);
            },
			showFormModal: function (item) {
				this.$refs.formModal.init(item);
				this.formModel.show = true;
            },
            doCopy: function (obj) {
				var _this = this;
				api.saveCopyTpl({id:obj.id}, {name: obj.name, orgId: obj.orgId, compId: obj.compId}).then(function (res) {
					LIB.Msg.info("复制成功");
                    _this.refreshMainTable();
                    // 如果详情页打开了
                    if(_this.detailModel.show) _this.$broadcast('ev_dtReload', "view", res.data.id);
                });
            }
        },
        events: {
			"do-show-copy": function (item) {
