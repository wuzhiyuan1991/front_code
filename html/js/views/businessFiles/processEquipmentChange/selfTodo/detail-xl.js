define(function (require) {
	...
	var dataModel = {
		mainModel: {
			...
			rules: {
				...
				"changeDeadline": [{
					validator: function (rule, val, callback) {
						...
						if (vo.changeMode == '1') {
							...
							vo.changeDeadline = null;
							_.set(vo, 'criteria.strValue.changeDeadline_empty', 1);
				...
				"otherInfluence": (LIB.formRuleMgr.require("是否有其他影响")),
				"benefitInfluence": (LIB.formRuleMgr.require("是否影响成本/效益")),
				"safetyInfluence": (LIB.formRuleMgr.require("是否影响安全健康")),
				"envInfluence": (LIB.formRuleMgr.require("是否影响环境")),
		...
		acceptance: {
			...
			rules: {
				...
				"acceptDate": [{
					validator: function (rule, val, callback) {
						var vo = dataModel.acceptance.vo;

						if (dataModel.isAcceptanceSubmit && _.isEmpty(vo.acceptDate)) {
							callback(new Error("变更验收日期"))
						} else {
							callback()
						}
		...
		useRangeEvaluation: {
			...
			rules: {
				...
				"ctrlMeasureResult": [{
					validator: function (rule, val, callback) {
						var vo = dataModel.useRangeEvaluation.vo;

						if (dataModel.isuseRangeEvaluationSubmit && _.isEmpty(vo.ctrlMeasureResult)) {
							callback(new Error("控制措施落实意见"))
						} else {
							callback()
						}
				...
				"dataUpdateResult": [{
					validator: function (rule, val, callback) {
						...
						if (dataModel.isuseRangeEvaluationSubmit && _.isEmpty(vo.dataUpdateResult)) {
							callback(new Error("资料更新落实意见"))
						} else {
							callback()
						}
		...
		implementation: {
			...
			rules: {
				...
				"implEndTime": [{
					validator: function (rule, val, callback) {
						if (dataModel.isImpSubmit && _.isEmpty(dataModel.implementation.vo.implEndTime)) {
							callback(new Error("实施结束时间"))
						} else if (dataModel.isImpSubmit && dataModel.implementation.vo.implStartTime >= dataModel.implementation.vo.implEndTime) {
				...
				"implStartTime": [{
					validator: function (rule, val, callback) {
						...
						if (dataModel.isImpSubmit && _.isEmpty(dataModel.implementation.vo.implStartTime)) {
							callback(new Error("实施开始时间"))
						} else {
							callback()
						}
					}
		...
		safeHealthy: [
			...
			{
				name: '是否有任何校准与否对安全非常重要的仪表？',
				checkResult: null
			},
			{
				name: '有无会给人员及设备造成危害的腐蚀物？',
				checkResult: null
			},
			{
				name: '是否存在空气排放问题？检查通风要求。',
				checkResult: null
			},
			{
				name: '是否存在有毒或危害性化学物品？如：石棉，放射性物质？检查通风要求。',
				checkResult: null
			},
			{
				name: '是否可能因蒸汽、仪表气源、电力或任何其它供应的中断而导致不安全状况？',
				checkResult: null
			},
			{
				name: '阀门位置错误对于安全问题是否重要？',
				checkResult: null
			},
			{
				name: '是否需要吹扫或惰性气体保护？',
			...
			{
				name: '是否需要通风孔、安全阀、溢出、排水或冲洗？检查其尺寸、位置 及供应商。',
				checkResult: null
			},
			{
				name: '是否需要冷冻保护？',
			...
			{
				name: '是否存在压力容器？泄压是否受变更方案的影响？',
				checkResult: null
			},
			{
				name: '不同压力等级的容器之间是否有连接？检查泄压要求。',
				checkResult: null
			},
			{
				name: '温度是否高或低至需要通过隔离来保护人员安全？',
				checkResult: null
			},
			{
				name: '温度是否高或低至可能造成管道或设备的热膨胀？',
				checkResult: null
			},
			{
				name: '是否需要围护措施以控制泄漏？检查尺寸。',
				checkResult: null
			},
			{
				name: '消防系统是否需要进行变更？',
			...
			{
				name: '是否存在任何机械伤害？通常需要添加围栏、扶手、机械防护或其 它操作人员保护设施等',
				checkResult: null
			},
			{
				name: '是否需因安全原因变更照明？考虑高空照明和应急照明。',
				checkResult: null
			},
			{
				name: '是否引入了任何可能改变电气分类的化学物品？',
				checkResult: null
			},
			{
				name: '静电或雷电是否会构成危害？',
			...
			{
				name: '拟变更结果是否会导致工作环境架高或造成空间封闭？',
				checkResult: null
			},
			{
				name: '对需要改进的控制器是否明确？经常用到或重要的仪器数值是否易 于读取，传输理解不会造成混淆？',
				checkResult: null
			},
			{
				name: '釆取控制动作后，是否能显示符合调整要求的画面？',
				checkResult: null
			},
			{
				name: '常用或重要的控制器和阀门是否能无应力正常操控？',
				checkResult: null
			},
			{
				name: '是否能够对误操作进行提示和纠正？',
				checkResult: null
			},
			{
				name: '是否考虑工作环境问题，即噪音、温度、照明等？',
				checkResult: null
			},
			{
				name: '是否考虑了操作和维修作业中的移动、姿势和可及度？人工加载任何新原料时是否考虑了材料对人体的影响？',
				checkResult: null
			},
			{
				name: '工作场所附近的事故应急设备是否满足需要？例如：将易燃物放入 非易燃物存放区后，应增加灭火器；或者使用腐蚀性化学物品的情 况下，应考虑洗眼站。',
				checkResult: null
			},
			{
				name: '在变更中，是否有可能使用或产生涉及国家法律规定应严格控制的 有毒物质？',
				checkResult: null
			},
		],
		airList: [
			...
			{
				name: '包含的化学品（仅列出那些增加、减少或者新出现的即可)：',
				checkResult: null
			},
			{
				name: '是否符合国家规定的有关气体排放标准？',
				checkResult: null
			},
			{
				name: '是否要求持续的排放监测/报告？',
			...
			{
				name: '增加工艺设备或者更换现有设备（包括工艺容器、辅助设备等等）？',
				checkResult: null
			},
			{
				name: '对控制方法进行物理或者操作变更，或者对现有设备增设控制装置。 这些改变是否会导致现有控制水平下降？',
				checkResult: null
			},
			{
				name: '提高生产率(按小时计，按年计)？',
			...
			{
				name: '使排放量增至高于实施控制前的*实际水平* (*前两年的平均排放 量）',
				checkResult: null
			},
			{
				name: '排放量减少？',
		...
		thingList: [
			...
			{
				name: '新产生或者增加的废弃物是否按合适方式处置？',
				checkResult: null
			},
			{
				name: '是否可以提供一份包含本项目涉及的所有化学品清单(仅列举那些 增 加、减少或者新增加的即可）',
				checkResult: null
			},
		],
		otherList: [
		...
		isuseRangeEvaluationSubmit: false,
		showOther: false,
		showThing: false,
		showSafe: false,
		showAir: false,
	...
	var detail = LIB.Vue.extend({
		...
		methods: {
			doChangeDataUpdateResult: function (val) {
				if (val == 0) { this.useRangeEvaluation.vo.dataUpdateDesc = '同意' } else if (val == 1) { this.useRangeEvaluation.vo.dataUpdateDesc = '不同意' }
			},
			doChangeCtrlMeasureResult: function (val) {
				if (val == 0) { this.useRangeEvaluation.vo.ctrlMeasureDesc = '同意' } else if (val == 1) { this.useRangeEvaluation.vo.ctrlMeasureDesc = '不同意' }
			},
			doDelete: function () {
			...
			doChange: function () {
				...
				this.isReturn = true
				if (this.mainModel.vo.pecDetail.safetyInfluence == 1) {
					this.safeDisabled = true
				}
				if (this.mainModel.vo.pecDetail.envInfluence == 1) {
			...
			doEntrust: function (vo) {
				...
				if (this.mainModel.vo.status == 1) {
					...
					data.operationType = 503
				} else if (this.mainModel.vo.status == 5) {
					data.operationType = 603
				}
			...
			afterInit: function () {
				...
				_.each(this.$refs, function (item) {
					if (item) {
						if (item.hasOwnProperty('resetFields')) {
							item.resetFields()
						}
					}
				...
				this.isAcceptanceSubmit = false
				this.isuseRangeEvaluationSubmit = false
				this.isImpSubmit = false
				this.isReturn = false
					...
					this.$api.queryPecAssessment({ id: this.mainModel.vo.id }).then(function (res) {
						if (res.data) {
							...
							if (_this.mainModel.vo.status > '1') {
								...
								_this.pecAssessment.opType = 'update'
							}
				...
				if (this.mainModel.vo.status > '2' && (this.mainModel.vo.bizType != 2 || this.mainModel.vo.level != 2)) {
					...
					this.$api.queryPecImplementation({ id: this.mainModel.vo.id }).then(function (res) {
						...
						if (res.data.hasOwnProperty('id')) {
							...
							api.getUUID().then(function (res) {
								_this.implementation.vo.id = res.data
			...
			doSubmituseRangeEvaluation: function () {
				...
				this.isuseRangeEvaluationSubmit = true
			...
			newVO: newVO,
			doPreview: function () {
				this.$emit("do-preview", {
					pecapplication: this.mainModel.vo,
			...
			safeClick: function (i, val, key) {
				...
				if (key == 'safeHealthy' && val == 1) {
					this.mainModel.vo.pecDetail.safetyInfluence = '1'
					this.safeDisabled = true
					...
					if (count > 0) {
						...
						this.mainModel.vo.pecDetail.safetyInfluence = null
						this.safeDisabled = false
					}
				}
				if (key !== 'safeHealthy' && val == 1) {
			...
			changeSafe: function (val, key, change) {
				...
				if (key !== 'safeHealthy' && val == 1) {
					this.mainModel.vo.pecDetail.envInfluence = '1'
					this.environmentDisabled = true
				} else if (key !== 'safeHealthy' && val !== 1) {
					var count = 0
					_.each(this.airList, function (item) {
					...
					if (count > 0) {
						...
						this.mainModel.vo.pecDetail.envInfluence = null
						this.environmentDisabled = false
					}
			...
			doUploadSuccess: function (param) {
				...
				if (this.implementation.vo.presentation) {
					...
					if (this.implementation.vo.presentation.indexOf('培训告知已完成') < 0) {
