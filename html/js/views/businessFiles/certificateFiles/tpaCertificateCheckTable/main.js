    ...
    var dataModel = function(){
		return{
			...
			tableModel: LIB.Opts.extendMainTableOpt(
				{
				url : "tpachecktable/list{/curPage}{/pageSize}?tableType=10",
				selectedDatas:[],
			...
			uploadModel: {
				url: "/tpachecktable/importExcel?type=10"
			},
			exportModel : {
				url: "/tpachecktable/exportExcel?type=10"
			},
            templete : {
                url: "/tpachecktable/file/down?type=10"
            },
            importProgress:{
                show: false
            }
		...
		methods:{
			...
			doDelete:function(){
				...
				if(rows.length > 1){
					...
					//this.findAuth({pojoName:"checktable",commandType:1,orgId:row.orgId},function(){
						LIB.Modal.confirm({
							title:'删除选中数据?',
							onOk:function(){
