define(function (require) {
    ...
    var opEmerStepItemFormModal = require("componentsEx/formModal/opEmerStepItemFormModal");
    ...
    var dataModel = {
        ...
        formModel: {
            stepFormModel: {
                show: false,
                hiddenFields: ["cardId"],
                queryUrl: "opemercard/{id}/opemerstep/{opEmerStepId}"
            },
            stepItemFormModel: {
                show: false,
                hiddenFields: ["cardId"],
                queryUrl: "opemerstep/{id}/opemerstepitem/{opemerstepitemId}"
            }
        },
        cardModel: {
            stepCardModel: {
    ...
    var detail = LIB.Vue.extend({
        ...
        template: tpl,
        components: {
            "stepFormModal": opEmerStepFormModal,
            "stepItemFormModal": opEmerStepItemFormModal,
        ...
        methods: {
            ...
            doShowStepFormModal4Update: function (data) {
                ...
                this.$refs.stepFormModal.init("update", {id: this.mainModel.vo.id, opEmerStepId: param.id});
            },
            doShowStepFormModal4Create: function (param) {
                this.formModel.stepFormModel.show = true;
            ...
            doRemoveSteps: function (data) {
                ...
                LIB.Modal.confirm({
                    title: '删除当前数据?',
                    onOk: function() {
                        api.removeOpEmerSteps({id: _this.mainModel.vo.id}, [{id: item.id}]).then(function () {
                            _this._getItems();
            ...
            doShowStepItemFormModal4Update: function (param) {
                ...
                this.$refs.stepItemFormModal.init("update", {id: param.entry.data.stepId, opemerstepitemId: param.entry.data.id});
            },
            ...
            doSaveStepItem: function (data) {
                if (data) {
                    ...
                    api.saveOpEmerStepItem({id: this._groupId}, data).then(function () {
                        _this._getItems();
            ...
            doUpdateStepItem: function (data) {
                if (data) {
                    var _this = this;
                    api.updateOpEmerStepItem({id: data.stepId}, data).then(function () {
                        _this._getItems();
                        LIB.Msg.success("修改成功");
                        _this._checkModifier();
                    });
            ...
            doRemoveStepItems: function (item) {
                ...
                LIB.Modal.confirm({
                    title: '删除当前数据?',
                    onOk: function() {
                        api.removeOpEmerStepItems({id: data.stepId}, [{id: data.id}]).then(function () {
                            _this._getItems();
                            LIB.Msg.success("删除成功");
                            _this._checkModifier();
                        });
            ...
            doMoveStepItems: function (item) {
                ...
                api.moveOpEmerStepItems({id: data.stepId}, param).then(function () {
                    _this._getItems();
                    LIB.Msg.success("移动成功");
                    _this._checkModifier();
                });
            ...
            _getItems: function () {
                ...
                api.getGroupAndItem({id: this.mainModel.vo.id}).then(function (res) {
                    var groups = res.data.OpEmerStep;
                    var items = res.data.OpEmerStepItem;
            ...
            _checkAuditProcess: function () {
                var _this = this;
                api.queryProcessStatus({auditObjectId: this.mainModel.vo.id, auditObjectType: 'OpEmerCard'}).then(function (res) {
            ...
            doShare: function() {
                ...
                if (data.disable === '1') {
                    return LIB.Msg.warning("未启用的应急处置卡不能进行分享操作");
                }
                var params = {
                    id: data.id,
            ...
            _checkProcessEnable: function () {
                ...
                api.queryLookupItem().then(function (res) {
                    var lookup = _.find(_.get(res.data, "[0].lookupItems"), "name", "OpEmerCard");
