    ...
    var detail = LIB.Vue.extend({
        ...
        methods:{
            ...
            getCraftsProcesses: function(){
                var _this = this;
                api.queryOpCraftsProcesses({pageNo: 1, pageSize: 999,id: this.id}).then(function (res) {
                    _this.craftsProcesses = res.data.list;
                    while(_this.craftsProcesses.length < 3){
                ...
                api.getGroupAndItem({id: this.id}).then(function (res) {
                    ...
                    var items = _.map(res.data.OpStdStepItem, function (item) {
                        return {
                            ...
                            ctrlMethod: item.ctrlMethod && item.ctrlMethod.replace(/[\r\n]/g, '<br/>')
                        }
                    });
