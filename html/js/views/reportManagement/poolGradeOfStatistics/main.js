    ...
    var opt = {
        ...
        methods:{
            ...
            buildPieChart:function(data){
                var _this = this;
                var opt = {
                    title:{x:'center',text:'隐患总数', top: 20},
                ...
                _.each(riskKeyValue[0],function (item) {
                ...
                api.poolCountAndReformCount(qryParam).then(function (res) {
                    if(res.data.length){
                        _this.mainModel.charts.pieChart.opt.title.text = "隐患总数：" + res.data[0].xValue + "    已整改总数："+res.data[0].yName+"    未整改总数："+res.data[0].yValue;
                    }
                })
                // api.poolCountAndReformCount(qryParam).then(function (res) {
                //     if(res.data.length){
                //         _this.mainModel.charts.pieChart.opt.title.text = "隐患总数：" + res.data[0].xValue;
                //     }
                // })
            },
