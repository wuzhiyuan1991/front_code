    ...
    var opt = {
        ...
        methods:{
            ...
            buildPieChart:function(data){
                var opt = {
                    title:{x:'center',text:'槽罐车作业安全巡检评价统计-饼状图',top:20},
            ...
            buildBarChart:function(data){
                var opt = {
                    title:{x:'center',text:'槽罐车作业安全巡检评价统计-柱状图',top:20},
            ...
            chartClick:function(params) {
                ...
                var result = null;
                if (params.data.name == "好评") {
                    result = 1;
                } else if (params.data.name == "中评") {
                    result = 2;
                } else if (params.data.name = "差评") {
                    result = 3;
                }
                //获取年
                ...
                var url = "rpt/stats/tpa/checkrecord/car/evaluate/{curPage}/{pageSize}?checkResult="+result;
                var tableOpt = {
                    url:url,
                    qryParam:qryParam,
                    columns:[
                        {title:"检查表",width:"280px",fieldName:"checktableName"},
                        {title:"数量",width:"150px",fieldName:"num"},
                        {title:"设备名称",width:"150px",fieldName:"equipmentName"}
                    ],
                }
            ...
            doQry:function(){
                ...
                api.checkRecorCountByEvaluate(qryParam).then(function(res){
                    _this.buildPieChart(res.data);
                });
                api.checkRecordCountByEvaluateDate(qryParam).then(function(res){
