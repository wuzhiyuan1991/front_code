    ...
    var vm = LIB.VueEx.extend({
        ...
        computed: {
            routeType: function () {
                ...
                else if(_.startsWith(path, '/complianceManagement')) {
        ...
        methods: {
            ...
            _getType: function () {
                ...
                var types = {
                    ...
                    'complianceManagement-1': 'certType',
                }
            ...
            _getURL: function (type, method) {
                var getURLs = {
                    industryCategory: api.industryCategory, // 行业分类
                    equipmentType: api.equipmentType, // 设备设施分类
                    courseCategory: api.courseCategory, // 课程类型
                    certificationSubject: api.certificationSubject, // 取证类型
                    riskType: api.risktype, // 检查项分类
                    tableType: api.listTableType, // 检查表分类
                    hazardFactor: api.listHazardFactor, // 危害因素分类
                    checkObjectCatalog: api.listCheckObjectCatalog, //重点化学工艺类型
                    checkObjectCatalogClassify: api.listCheckObjectCatalogClassify, //化学品类别
                    checkBasisType: api.checkbasistype, // 检查依据分类
                    certType: api.listCertType // 证书类型
                };

                var deleteURLs = {
                    riskType: api.deleteRiskType,
                    hazardFactor: api.delHazardFactor,
                    checkBasisType: api.deleteCheckBasisType,
                    tableType: api.delTableType,
                    equipmentType: api.delEquipmentType,
                    industryCategory:api.delIndustryCategory,
                    courseCategory: api.delCourseCategory,
                    certificationSubject: api.delCertificationSubject,
                    checkObjectCatalog: api.delCheckObjectCatalog,
                    checkObjectCatalogClassify: api.delCheckObjectCatalogClassify,
            ...
            _getData: function (isHazardFactor) {
                ...
                apiURL(param).then(function (res) {
            ...
            doDeleteNode: function (value) {
                ...
                if (_.includes(c2, type)) {
                    ...
                    LIB.Modal.confirm({
                        title: '删除当前数据?',
                        onOk: function() {
                            apiURL(null, param).then(callback1);
                        }
                    });
                }
