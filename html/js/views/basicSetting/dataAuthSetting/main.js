    ...
    var dataModel = {
        ...
        assistFunc: function (item) {
            if(item.code || item.key || item.disabled) {
                return '<span style="display: inline-block;width: 180px;cursor: default;">' + item.code + '</span><span style="display: inline-block;cursor: default;width: 150px;">' + item.key + '</span><span style="display: inline-block;cursor: default;">' + (item.disable === '0' ? '启用' : '停用') + '</span>';
            }
            return '';
        }
    };
