define(function (require) {
    ...
    var tpl = require("text!./edit-role.html");
    ...
    var detail = LIB.Vue.extend({
        ...
        methods: {
            doSave: function () {
                ...
                var callback = function (res) {
                    var roleData = [];
                    //循环获取选中的key值name 用来更新角色数据
                    _.each(_this.mainModel.vo.list,function(roleId){
                        _.each(_this.mainModel.rList,function(roleList){
                            if(roleId == roleList.id ){
                                roleData.push(roleList);
                            }
                        })
                    })
                 ...
                 if(dataModel.mainModel.role == "role"){
                   ...
                   //_this.$dispatch("ev_editRoleFinshed",roleData);
                    _this.$emit("do-edit-role-finshed",roleData);
                    LIB.Msg.info("保存成功");
                };
                ...
                api.distributionRole(_.omit(this.mainModel.vo, "roleList")).then(callback);
            },
        ...
        events: {
            //edit框数据加载
            "ev_editRole": function (nVal,role) {
                var _this=this
                ...
                api.listRole().then(function (res) {
                    _data.rList = res.data;
                     if (nVal != null) {
                    api.get({id: nVal}).then(function (res) {
                        _vo.roleList = res.data.roleList;
                        var userDataLeft = [],userDataRigth = [];
                        //整理数据 左边的数据
                        _.each(_data.rList,function(item){
                           var dataLeft={
                                key:item.id,
                                label: item.name
                            }
                        ...
                        //整理数据
                        _.each(_vo.roleList,function(item){
                           var dataRigth={
                                key:item.id
                            }
