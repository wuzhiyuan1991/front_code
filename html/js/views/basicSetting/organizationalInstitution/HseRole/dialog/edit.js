    ...
    var detail = LIB.Vue.extend({
        ...
        methods: {
            doSave: function () {
                var _this = this;
                //如果有部门就传部门
                //if(_this.mainModel.vo.orgIdData){
                    //_this.mainModel.vo.orgId = _this.mainModel.vo.perId;
                    _this.mainModel.vo.orgId = _this.mainModel.vo.compId;
               // }
                ...
                this.$refs.ruleform.validate(function (valid) {
                    if (valid) {
                        if (_this.mainModel.opType == "create") {
                            //api.create(_.omit(_this.mainModel.vo, "id", "posList", "orgList","postType","orgId")).then(callback);
                            api.create(_.omit(_this.mainModel.vo,  "id", "posList", "orgList","orgIdData","perId","organizationId")).then(callback);
                        } else {
                           // api.update(_.omit(_this.mainModel.vo, "posList", "orgId")).then(callback);
                            api.update(_.pick(_this.mainModel.vo,  "id","code", "name", "orgId","compId","postType","remarks")).then(callback);
                        }
                    } else {
                        return false;
                    }
                    ...
                    api.listDept({type: 2}).then(function (res) {
                        ...
                        if (nVal != null) {
                            ...
                            api.get({id: nVal}).then(function (res) {
                                ...
                                //_vo.compId = dataDicValuevar.id;
                                _vo.compId = res.data.compId;
