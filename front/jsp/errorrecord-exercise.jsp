...
<head>
    ...
    <script type="text/javascript" src="/static/script/errorrecord-exercise.js"></script>
    ...
    <script>
        $(function () {
            ...
            $('input[type=radio]').click(function () {
                var index = $(this).parent().attr("numinder");
                if (index) {
                    var current = $('#datikaCurrent' + index);
                    var next = $('#datikaCurrent' + (+index + 1));
                    if(next && current.attr('catalog') === next.attr('catalog')) {
                        next.click();
                    }
                }
            });
        ...
        //答题卡点击
        function datikaAnchor(titleNum, num) {
            // 如果题型不匹配，则先切换到对应的题型
            if(titleNum !== titleheiddenNum) {
                $("[indexnum=" + titleNum +"]").click();
            }

            if ($(".datikaQstAnchor" + num).length > 0) {
                $("html").animate({scrollTop: $(".datikaQstAnchor" + num).offset().top - 200}, 500);
            } else {
                $("html").animate({scrollTop: $(".datikaQstAnchor" + (num - 1)).offset().top - 200}, 500);
            }
        }
...
<body>
<form action="${ctx}/front/exerciseover?_bizModule=errorRecord" method="post" id="addExerciseRecord">
                                        ...
                                        <table class="ques-card-tab">
                                            ...
                                            <c:forEach items="${topicList }" varStatus="topicListindex" var="paperMiddle">
                                                ...
                                                <c:forEach items="${paperMiddle.questions}" varStatus="qstMiddleIndex">
                                                    ...
                                                    <td onclick="datikaAnchor(${topicListindex.index},<c:out value="${daTiKaNumIndex}"/>)" id="datikaCurrent<c:out value="${daTiKaNumIndex}"/>" catalog='<c:out value="${topicListindex.index}"/>'>
                                                        <a href="javascript:void(0)" title="第<c:out value="${daTiKaNumIndex}"/>题"><c:out value="${daTiKaNumIndex}"/></a>
                                                    </td>
                        ...
                        <ul class="ques-type mt20 clearfix">
                            <c:forEach items="${topicList }" varStatus="index" var="paperMiddle">
                                <li <c:if test="${index.first}">class="current"</c:if> >
                                    <a href="javascript:void(0)" class="titleHeiddenAndShow${index.index}" indexNum="${index.index }" title="${name }"
                                       titleValue="${paperMiddle.title}[&nbsp;每小题${topicList[index.index].score }分&nbsp;]"
                                       onclick="titleHeiddenAndShow('${paperMiddle.id }',this)">
                                            ${paperMiddle.name }
                                    </a>
                                </li>
                            </c:forEach>
                            ...
                            <div class="p-ques-list">
                                <c:forEach items="${topicList}" varStatus="topicListindex" var="paperMiddle">
                                    ...
                                    <ul id="titleHidden${paperMiddle.id }"
                                        <c:if test="${!topicListindex.first }">style="display:none"</c:if> >
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${qstMiddle.type==1||qstMiddle.type==3}">
                                                                    <label for="<c:out value="${questionNumIndex}"/>_${option.opt}" numInder="<c:out value="${questionNumIndex}"/>">
                                                                        ...
                                                                        <input type="radio" name="recordList[${numIndex}].userAnswer"
                                                                               id="<c:out value="${questionNumIndex}"/>_${option.opt }" value="${option.opt }"/>
                                                                        <span>${option.opt }</span>
                                                                    </label>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <label for="<c:out value="${questionNumIndex}"/>_${option.opt }"
                                                                           numInder="<c:out value="${questionNumIndex}"/>">
                                                                        <em class="icon18 o-checkbox">&nbsp;</em>
                                                                        <input type="checkbox" name="recordList[${numIndex}].userAnswer"
                                                                               id="<c:out value="${questionNumIndex}"/>_${option.opt }" value="${option.opt }"/>
                                                                        <span>${option.opt}</span>
                                                                    </label>
                                                                </c:otherwise>
